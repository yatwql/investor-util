...
class TestSP1DirectStocks(unittest.TestCase):
    ...
    def _make_detail(self, code: str, name: str, price: float, shares: int = 100) -> DetailRow:
        return DetailRow(
            account="证券",
            name=name,
            code=code,
...
class TestSP3ETF(unittest.TestCase):
    ...
    def test_foreign_index_etf_classification(self, mock_enrich, mock_batch):
        ...
        self.assertIn(
            "QDII", result["summary"]["fund_breakdown"], "标普500ETF 应分类为 QDII（海外指数 ETF 实质为 QDII 产品）"
        )
        # 来源标签应为 [QDII]
        for entry in result["top10"]:
...
class TestSP4ActiveEquity(unittest.TestCase):
    ...
    def test_active_equity_sector_medical(self, mock_ind, mock_batch):
        ...
        result = pene.compute_penetration_top10(self.holdings, details)
        for entry in result["top10"]:
            self.assertEqual(entry["sector"], "医药", f"{entry['name']} 板块应为医药")

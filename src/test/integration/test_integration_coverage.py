...
覆盖项：
  ...
  6. Excel/HTML 章节一致性  — 两端可见章节集合/名称/顺序与注册表三方一致
...
# ═════════════════════════════════════════════════════════════════════════
#  6. Excel / HTML / 注册表 三方章节一致性验证 (integration_contract)
# ═════════════════════════════════════════════════════════════════════════
...
class TestReportChapterConsistency(unittest.TestCase):
    ...
    锁死两条防漂移保证：
      1. 同一逻辑输入（board 开关 + 数据就绪状态）下，Excel create_sheets 与
         HTML _compute_section_visibility 产出**相同的可见章节集合与顺序**；
    ...
    def test_excel_sheets_match_registry_order_all_enabled(self):
        ...
        self.assertEqual(len(excel_keys), 19)
    ...
    def test_html_visible_numbers_match_registry_all_data_present(self):
        """数据全就绪时 HTML 连续编号 1..19 == 注册表顺序，llm_usage 编号 19。"""
        from src.python.core.registry import get_report_section_order
        from src.python.report.html_writer import _compute_section_visibility
        ...
        self.assertEqual(list(visible_numbers.values()), list(range(1, 20)), "HTML 连续编号应为 1..19")
        self.assertEqual(visible_numbers["llm_usage"], 19, "llm_usage 编号必须为 19（末位）")

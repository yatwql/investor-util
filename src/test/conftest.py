...
from src.test._path_isolation import (
    apply_report_output_isolation,
    seed_sensitive_path_isolation,
)
...
def _isolate_sensitive_paths(tmp_path, monkeypatch, _doctor_probe_targets):
    ...
    """
    seed_sensitive_path_isolation(monkeypatch, tmp_path, _doctor_probe_targets)
...
def _isolate_report_output_dir(tmp_path, monkeypatch):
    ...
    """
    apply_report_output_isolation(monkeypatch, tmp_path)
...
def offline_external_sources(monkeypatch):
    ...
    作用：把“附带依赖”的外部数据入口置为**离线**，不再依赖全局网络守卫兜底。
    报告/编排类用例通常已 mock 了自己关注的数据（持仓/新闻/LLM），而生产链路还会
    顺带查询交易日历、行业数据、行情、健康检查探针等；这些附带依赖经本 fixture
    变为“即时取不到”——降级/可用性口径不变，但零网络、零等待（仅靠守卫兜底时，
    每个 provider 槽会白等 0.6~1.2s 的链路退避）。

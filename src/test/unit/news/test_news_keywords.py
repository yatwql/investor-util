...
class TestBuildHoldingKeywords(unittest.TestCase):
    ...
    def test_etf_name_clean(self) -> None:
        """ETF 名称去掉后缀后仍能提取核心词。"""
        holdings = [
            Holding(account="证券", name="沪深300ETF", code="510300", shares=100, cost_price=4.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_fund_clean(self) -> None:
        """基金名称去掉后缀后提取核心词。"""
        holdings = [
            Holding(account="基金", name="易方达蓝筹精选混合", code="005827", shares=1000, cost_price=2.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_lianjie_handling(self) -> None:
        """联接基金 → 拆分提取母基金名称。"""
        holdings = [
            Holding(account="基金", name="天弘沪深300ETF联接A", code="000961", shares=1000, cost_price=1.5),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_qdii_suffix_stripped(self) -> None:
        """QDII 后缀被剥离。"""
        holdings = [
            Holding(account="基金", name="华夏恒生ETF联接(QDII)A", code="000071", shares=1000, cost_price=1.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_penetrated_assets_added(self) -> None:
        """穿透资产名称和代码被加入关键词。"""
        holdings = [Holding(account="证券", name="贵州茅台", code="600519", shares=100, cost_price=200.0)]
        penetrated = [
    ...
    def test_english_asset_name_added(self) -> None:
        """纯英文穿透资产名直接添加。"""
        holdings = [Holding(account="证券", name="标普500", code="SPY", shares=100, cost_price=400.0)]
    ...
    def test_max_keywords_limit(self) -> None:
        """max_keywords 限制返回数量。"""
        holdings = [
            Holding(account="证券", name=f"股票{i:04d}", code=f"600{i:04d}", shares=100, cost_price=10.0)
            for i in range(20)
        ]
    ...
    def test_duplicates_removed(self) -> None:
        ...
        holdings = [
            Holding(account="证券", name="长江电力", code="600900", shares=100, cost_price=10.0),
            Holding(account="证券", name="长江电力", code="600900", shares=200, cost_price=12.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_short_terms_filtered(self) -> None:
        ...
        holdings = [
            Holding(account="证券", name="A股", code="000001", shares=100, cost_price=10.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_empty_code_skipped(self) -> None:
        """代码为空时跳过。"""
        holdings = [
            Holding(account="证券", name="现金", code="", shares=100, cost_price=1.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_sort_by_length_desc(self) -> None:
        """关键词按长度降序排列（长关键词优先匹配）。"""
        holdings = [
            Holding(account="证券", name="易方达蓝筹精选混合", code="005827", shares=100, cost_price=2.0),
            Holding(account="证券", name="长江电力", code="600900", shares=100, cost_price=10.0),
        ]
        keywords = build_holding_keywords(holdings)
        # 长词应在短词前面
    ...
    def test_empty_name_with_code(self) -> None:
        """名称为空但代码存在 → 只提取代码。"""
        holdings = [
            Holding(account="证券", name="", code="600900", shares=100, cost_price=10.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_empty_name_and_empty_code(self) -> None:
        """名称和代码均为空 → 返回空列表。"""
        holdings = [
            Holding(account="证券", name="", code="", shares=100, cost_price=10.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_name_special_chars(self) -> None:
        ...
        holdings = [
            Holding(account="证券", name="药明康德(港股)", code="02359", shares=100, cost_price=10.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_name_with_dash(self) -> None:
        """名称含破折号 → 正确提取中文关键词。"""
        holdings = [
            Holding(account="证券", name="ST-华英", code="002321", shares=100, cost_price=10.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_name_only_ascii(self) -> None:
        """名称只有 ASCII 字符 → 只提取代码。"""
        holdings = [
            Holding(account="美股", name="Apple Inc.", code="AAPL", shares=100, cost_price=150.0),
        ]
        keywords = build_holding_keywords(holdings)
    ...
    def test_lianjie_with_single_part(self) -> None:
        ...
        holdings = [
            Holding(account="基金", name="联接A", code="000000", shares=100, cost_price=1.0),
        ]
        keywords = build_holding_keywords(holdings)

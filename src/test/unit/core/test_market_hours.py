...
class TestIsMarketOpen(unittest.TestCase):
    ...
    @patch("src.python.core.market_hours.datetime")
    def test_config_takes_precedence_true(self, mock_dt, mock_fallback, mock_official, mock_config):
    ...
    @patch("src.python.core.market_hours._is_market_open_fallback")
    @patch("src.python.core.market_hours.datetime")
    def test_official_takes_precedence(self, mock_dt, mock_fallback, mock_official, mock_config):
    ...
    @patch("src.python.core.market_hours.datetime")
    def test_fallback_used_when_all_others_none(self, mock_dt, mock_fallback, mock_official, mock_config):
    ...
    @patch("src.python.core.market_hours._is_market_open_fallback")
    @patch("src.python.core.market_hours.datetime")
    def test_config_false_short_circuits(self, mock_dt, mock_fallback, mock_official, mock_config):
    ...
    @patch("src.python.core.market_hours._is_market_open_fallback")
    @patch("src.python.core.market_hours.datetime")
    def test_exception_returns_false(self, mock_dt, mock_fallback, mock_official, mock_config):
        """异常时保守返回 False。"""
        mock_dt.now.return_value = datetime(2026, 7, 1, 10, 0, tzinfo=timezone(timedelta(hours=8)))
        mock_dt.timezone = timezone
        mock_dt.timedelta = timedelta
...
class TestUtcTimezoneConsistency(unittest.TestCase):
    ...
    def _test_at_time(self, beijing_hour: int, beijing_min: int, weekday: int, system_tz_offset: int) -> bool:
    ...
    def _call_is_market_open_at_beijing_time(
        self, hour: int, minute: int, weekday: int = 0, system_offset: int = 8
    ) -> bool:
        """简化的北京时间测试辅助方法。"""
        with patch("src.python.core.market_hours.datetime") as mock_dt:
            # 构造北京时间的时间对象
            beijing_dt = datetime(2026, 7, 6 + weekday, hour, minute, tzinfo=timezone(timedelta(hours=8)))
            # 对应 UTC 时间
    ...
    def setUp(self):
        # 确保 config 层不干预
        self._cfg_patcher = patch("src.python.core.market_hours._is_market_open_config", return_value=None)
        self._cfg_patcher.start()
        self._official_patcher = patch("src.python.core.market_hours._is_market_open_official", return_value=None)
    ...
    def test_utc_system_at_beijing_morning(self):
        ...
        self.assertTrue(self._call_is_market_open_at_beijing_time(10, 0, weekday=1, system_offset=0))

    def test_utc_system_at_beijing_lunch(self):
        """系统 UTC，北京时间 12:00 周二 → False（午休）。"""
        self.assertFalse(self._call_is_market_open_at_beijing_time(12, 0, weekday=1, system_offset=0))

    def test_utc_system_at_beijing_afternoon(self):
        """系统 UTC，北京时间 14:00 周二 → True。"""
        self.assertTrue(self._call_is_market_open_at_beijing_time(14, 0, weekday=1, system_offset=0))

    def test_utc_system_at_beijing_closed(self):
        """系统 UTC，北京时间 15:30 周二 → False。"""
        self.assertFalse(self._call_is_market_open_at_beijing_time(15, 30, weekday=1, system_offset=0))

    # ── 系统时区为 EST（UTC-5）─ 北京时间 10:00 交易日 ──

    def test_est_system_at_beijing_morning(self):
        """系统 EST(UTC-5)，北京时间 10:00 周二 → True。"""
        self.assertTrue(self._call_is_market_open_at_beijing_time(10, 0, weekday=1, system_offset=-5))

    def test_est_system_at_beijing_lunch(self):
        """系统 EST(UTC-5)，北京时间 12:00 周二 → False（午休）。"""
        self.assertFalse(self._call_is_market_open_at_beijing_time(12, 0, weekday=1, system_offset=-5))

    def test_est_system_at_beijing_weekend(self):
        """系统 EST(UTC-5)，北京时间周六 10:00 → False。"""
        self.assertFalse(self._call_is_market_open_at_beijing_time(10, 0, weekday=5, system_offset=-5))

    # ── 系统时区为 UTC+8（北京时间）─ 基线验证 ──

    def test_cst_system_at_beijing_morning(self):
        """系统 UTC+8，北京时间 10:00 周二 → True（基线）。"""
        self.assertTrue(self._call_is_market_open_at_beijing_time(10, 0, weekday=1, system_offset=8))

    def test_cst_system_at_beijing_lunch(self):
        """系统 UTC+8，北京时间 12:00 周二 → False（基线）。"""
        self.assertFalse(self._call_is_market_open_at_beijing_time(12, 0, weekday=1, system_offset=8))

    def test_cst_system_weekend(self):
        """系统 UTC+8，北京时间周六 10:00 → False（基线）。"""
        self.assertFalse(self._call_is_market_open_at_beijing_time(10, 0, weekday=5, system_offset=8))

    # ── 边界日期：跨 UTC 日期线 ──

    def test_utc_system_cross_date_line_morning(self):
        """系统 UTC，北京时间周一 09:30（UTC 周日 01:30）→ True（交易日）。"""
        self.assertTrue(self._call_is_market_open_at_beijing_time(9, 30, weekday=0, system_offset=0))

    def test_utc_system_cross_date_line_weekend_beijing_saturday(self):
        """系统 UTC，北京时间周六 09:30（UTC 周五 01:30）→ False（周末）。"""
        self.assertFalse(self._call_is_market_open_at_beijing_time(9, 30, weekday=5, system_offset=0))

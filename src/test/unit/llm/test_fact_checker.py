...
class TestTrimTargetContext:
    ...
    def test_condition_threshold_over_pct_not_corrected(self, real_holdings):
        ...
        真实缺陷复现（穿透深度分析 LLM 输出）：原文「建设银行收益率+180.55%、
        长江电力+59.21%…但在收益率超过 200% 后可考虑部分止盈锁定利润」——
        200% 是止盈目标阈值，非对 600900 当前收益率的陈述。旧逻辑因"止盈"
        在数值之后较远处（超出 _TRIM_TARGET_KEYWORDS 的 [-15,+5] 邻近窗口），
        未命中止盈目标语境，误把 200% 归因到最近名称"长江电力(600900)"并
        修正为 56.8% —— 把原本正确的条件表述改错。
        """
        text = (
...
class TestNameAliasNormalized:
    ...
    真实缺陷复现（辩论综合 LLM 输出）：「本质分歧在于如何处理已实现的巨额浮盈
    （华安纳指+180.5%、建设银行+180.55%）」——华安纳指（040046）实际收益率
    130.61%，LLM 反向串位写成 180.5%。旧逻辑 _locate_subject_code 仅按持仓
    全名匹配，"华安纳指"匹配不到"华安纳斯达克100ETF联接基金A" → 主体定位
    失败 → 回退全局最近邻，180.5 恰好命中 601939 真实值 180.55 → 误判通过，
    反向串位漏检。
    """

...
def _make_detail_row(code: str, market_value: float = 1000.0, name: str = "持仓") -> "DetailRow":
...
class TestPenetrationDataDeduplication(unittest.TestCase):
    ...
    def test_same_code_in_fund_and_direct(self):
        ...
        merged = {
            "宁德时代": _make_merged_item("宁德时代", 30000, funds=["直接持股", "沪深300ETF"]),
        }
        classified = {"qdii": [], "etf": [], "index_link": [], "bond_fund": [], "active_equity": []}
    ...
    def test_same_code_multiple_funds_deduped(self):
        ...
        merged = {
            "贵州茅台": _make_merged_item("贵州茅台", 50000, codes=["600519"], funds=["易方达蓝筹", "沪深300ETF"]),
        }
        classified = {"qdii": [], "etf": [], "index_link": [], "bond_fund": [], "active_equity": []}
        result = _build_penetration_result(
            merged=merged,
...
class TestETFSuperManyHoldings(unittest.TestCase):
    ...
    def test_etf_200_holdings_no_crash(self, mock_batch, mock_ind):
        ...
        holdings_200 = [{"name": f"持仓{i:04d}", "code": f"{600000 + i}", "ratio": 0.5} for i in range(200)]

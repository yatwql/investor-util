...
class _MockDetailRow:
    ...
    def __init__(self, code: str, profit: float = 0.0, profit_rate: float = 0.0, market_value: float = 0.0):
        self.code = code
        self.profit = profit
...
class TestIsFund(unittest.TestCase):
    ...
    def _h(self, name: str, code: str = "", account: str = "证券账户") -> Holding:
        return Holding(account=account, name=name, code=code, shares=1.0, cost_price=1.0)
    ...
    def test_non_stock_prefix_codes(self):
        """非 A 股/港股通前缀代码 -> True"""
        cases = [("某债券", "110011"), ("某ETF", "510050"), ("某产品", "400000")]
...
class TestFormatRank(unittest.TestCase):
    ...
    def test_rank_zero(self):
        """rank=0 -> '0/total'"""
        self.assertEqual(fp._format_rank({"rank": 0, "total": 500}), "0/500")

    def test_string_values(self):
        """字符串数字值 -> 'rank/total'"""
        self.assertEqual(fp._format_rank({"rank": "5", "total": "100"}), "5/100")

    def test_large_numbers(self):
        """大数字 -> '1234/99999'"""
        self.assertEqual(fp._format_rank({"rank": 1234, "total": 99999}), "1234/99999")
...
class TestRatingComment(unittest.TestCase):
    ...
    def test_description_prefix_matches_key(self):
        ...
        for k, v in fp._RATING_COMMENT.items():
            with self.subTest(key=k):
                self.assertTrue(v.startswith(k), f"'{v}' 不以 '{k}' 开头")
...
class TestWriteFundPerformanceSheet(unittest.TestCase):
    ...
    _PATCH_TARGETS = [
        ...
        "src.python.report.fund_performance.fetch_fund_rankings_batch",
    ...
    def setUp(self):
        ...
        def _rankings_batch_side_effect(codes: list[str]) -> dict:
            return {code: _RANKINGS_DATA.get(code) for code in codes}

        self.mocks["fetch_fund_rankings_batch"].side_effect = _rankings_batch_side_effect

        # -- fetch_fund_benchmark 侧效 --
        def _benchmark_side_effect(code: str) -> str:
            return {"561910": "沪深300指数", "012325": "中债综合指数"}.get(code, "--")
        ...
        self.details = [
            _MockDetailRow("561910", profit=1000.0, profit_rate=0.05, market_value=10000.0),
            _MockDetailRow("012325", profit=200.0, profit_rate=0.03, market_value=5000.0),
            _MockDetailRow("600900", profit=500.0, profit_rate=0.10, market_value=10000.0),
        ]
    ...
    def test_normal_flow(self):
        ...
        # --- 统计行 ---
        vals2 = calls[2][0][2]
        self.assertEqual(vals2[0], "共 2 只基金，2 只获取到业绩数据")
        ...
        self.mocks["auto_width"].assert_called_once_with(self.ws, min_width=10, max_width=30)
    ...
    def test_no_funds(self):
        ...
        self.mocks["write_title_row"].assert_called_once_with(self.ws, 1, "基金业绩分析", 11)
        ...
        # fetch 不应被调用
        self.mocks["fetch_fund_rankings_batch"].assert_not_called()
    ...
    def test_all_funds_fail(self):
        ...
        self.mocks["fetch_fund_rankings_batch"].side_effect = lambda codes: {code: None for code in codes}
        ...
        self.assertEqual(summary_vals[0], "共 2 只基金，0 只获取到业绩数据")
    ...
    def test_partial_fail(self):
        ...
        # 只有 561910 成功
        self.mocks["fetch_fund_rankings_batch"].side_effect = lambda codes: {
            code: (
                None
                if code == "012325"
                else {
    ...
    def test_rating_colors(self):
        ...
        def _rankings_all_ratings(codes: list[str]) -> dict:
            data = {
                "561910": {
                    "rankings": {
                        "近3月": {"return": 1.0},
        ...
        self.mocks["fetch_fund_rankings_batch"].side_effect = _rankings_all_ratings
        ...
        details = self.details + [
            _MockDetailRow("005827", profit=-100.0, profit_rate=-0.02, market_value=3000.0),
        ]
...
class TestRankDataReasonableRange(unittest.TestCase):
    ...
    @patch("src.python.report.fund_performance.fetch_fund_benchmark", return_value="沪深300指数")
    @patch("src.python.report.fund_performance.classify_penetration", return_value=pene.ACTIVE_EQUITY)
    def test_yield_rate_in_reasonable_range(self, mock_pene, mock_bm, mock_batch):
        ...
        details = [_MockDetailRow("005827", profit=1000.0, profit_rate=0.05, market_value=10000.0)]
        with patch("src.python.report.fund_performance.write_data_row") as mock_wdr:
            with patch("src.python.report.fund_performance.write_title_row", return_value=2):
                with patch("src.python.report.fund_performance.write_header_row", return_value=3):

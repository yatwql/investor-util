...
class TestDegradationSignal1:
    ...
    def test_signal1_exceeds_threshold(self):
        ...
        for _ in range(2):
            tracker.record(
                "test_source", "T2", success=False, failure_type="unreachable", cache_age_hours=12, cache_ttl_hours=24
            )
        degraded, count, effective = tracker.record(
            "test_source",
            "T2",
    ...
    def test_signal1_exceeds_at_fourth(self):
        ...
        tracker = DegradationTracker()
        for _ in range(4):
            tracker.record("test_source", "T2", success=False, failure_type="unreachable")
    ...
    def test_signal1_t3_higher_empty_threshold(self):
        ...
        for _ in range(2):
            tracker.record(
                "test_source", "T3", success=False, failure_type="empty", cache_age_hours=12, cache_ttl_hours=24
            )
...
class TestDegradationFailureType:
    ...
    def test_empty_data_higher_threshold(self):
        ...
        for _ in range(2):
            tracker.record(
                "test_source", "T2", success=False, failure_type="empty", cache_age_hours=12, cache_ttl_hours=24
            )
        # 第 3 次仍小于 empty 有效阈
        degraded, count, effective = tracker.record(
            "test_source",
        ...
        for _ in range(2):
            tracker.record(
                "test_source2", "T2", success=False, failure_type="unreachable", cache_age_hours=12, cache_ttl_hours=24
            )
    ...
    def test_success_resets_all_counters(self):
        ...
        tracker = DegradationTracker()
        tracker.record("test_source", "T2", success=False, failure_type="unreachable")
        tracker.record("test_source", "T2", success=False, failure_type="empty")
        # 成功
...
class TestDegradationThreadSafety:
    ...
    def test_concurrent_records(self):
        ...
        def _fail(idx: int):
            tracker.record(f"source_{idx % 3}", "T2", success=False, failure_type="unreachable")

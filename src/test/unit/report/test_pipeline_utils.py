...
class TestPipelineUtils:
    ...
    def test_report_llm_module_results_all_success(self):
        """全部成功 → ok 消息。"""
        from src.python.report._pipeline import _report_llm_module_results
    ...
    def test_process_llm_future_success(self):
        """正常结果 → 正确解包。"""
        from src.python.report._pipeline import _process_llm_future
        ...
        llm_content, cached, debate_info = _process_llm_future(fut, _MockReporter())
        assert llm_content[:4] == content
        assert debate_info == {"mode": "procon"}

    def test_process_llm_future_exception(self):
        """异常 → 返回 (全 None, 全 False, None)。"""
        from src.python.report._pipeline import _process_llm_future
        ...
        assert all(not c for c in cached)
        assert debate_info is None

    def test_process_news_future_success(self):
        """正常结果 → 返回 (data, meta, ok)。"""
        from src.python.report._pipeline import _process_news_future
        ...
        data, meta, ok = _process_news_future(fut, _MockReporter())
        assert len(data) == 1
        assert meta == {"total": 1}
        assert ok is True

    def test_process_news_future_exception(self):
        """异常 → 返回 ([], {}, False)。"""
        from src.python.report._pipeline import _process_news_future

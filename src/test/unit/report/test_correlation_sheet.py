...
class TestExcelCorrelationSheet(unittest.TestCase):
    ...
    def test_matrix_pairs_and_notes_rendered(self):
        ...
        self.assertTrue(any("相关性矩阵" in t or "持仓相关性" in t for t in titles),
                        f"应含相关性标题，实际: {titles}")
        self.assertTrue(any("配对明细" in t for t in titles),
                        f"应含配对明细标题，实际: {titles}")
    ...
    def test_unavailable_placeholder(self):
        ...
        ws = self._write(data)
        flat = self._flat(ws)
        self.assertTrue(any("持仓相关性数据暂不可用" in v for v in flat),
                        "available=False 应写占位")
    ...
    def test_pairs_sorted_by_abs_r(self):
        ...
        data["pairs"] = [
            {"code_a": "c", "name_a": "资产C", "code_b": "a", "name_b": "资产A",
             "pearson": 0.9, "p_value": 0.001, "significant": True, "samples": 60},
            {"code_a": "b", "name_a": "资产B", "code_b": "a", "name_b": "资产A",
             "pearson": -0.2, "p_value": 0.1, "significant": False, "samples": 60},
            {"code_a": "c", "name_a": "资产C", "code_b": "b", "name_b": "资产B",
             "pearson": 0.1, "p_value": 0.2, "significant": False, "samples": 60},
        ]
        ws = self._write(data)
        ...
        self.assertEqual(pearson_vals,
                         sorted(pearson_vals, key=abs, reverse=True),
                         "配对应严格按 |r| 降序")
        # 显著标记出现在 0.9 那对

...
约束（必须遵守）：只引用数据中实际存在的品种，不虚构任何数据。每个判断必须有数据支撑。不得编造任何收益率、百分比或排名数值——输入未提供的数值不得自行生成。整体百分比（如盈利品种占比等）须有实际计数支撑。"""
...
约束（必须遵守）：
- 数据来自输入，**不得虚构任何数值、百分比或排名**
- 每个论点引用的收益率、占比等数值必须是输入中实际提供的
- 不知输入中未提供"最大持仓"的排名信息时，不得断言 XX 是"最大持仓"或"第一重仓"
- 不知品种具体收益率时，不得编造收益率数字
- 整体组合盈利比例（如"XX% 品种处于盈利"）需有实际计数支撑，不编造"""
...
约束（必须遵守）：
- 数据来自输入，**不得虚构任何数值、百分比或排名**
- 每个论点引用的收益率、占比、集中度等数值必须是输入中实际提供的
- 不得编造品种的估值数据（PE、PB 等）——如输入未提供，提示"估值数据未提供"即可
- 不知具体数字时使用定性描述（"偏高/偏低"）而非虚构具体数值"""

_SYSTEM_DEBATE_SYNTHESIS = """你是投资智囊团首席指挥官。以下是两份由不同角色生成的独立分析报告：
...
请综合正反两份报告，按以下步骤输出最终投资建议：

1. **关键分歧点** — 白脸和黑脸在哪些问题上观点对立（列出3-5个）
2. **分歧定论** — 对每个分歧点给出你的判断，并说明理由
3. **综合行动建议** — 结合正反两面，给出可执行的调仓操作建议（分优先级别）
4. **置信度评级** — 对每条建议标注置信度（高/中/低），低置信度的建议请附加跟踪条件

输出格式：使用表格对比分歧点，行动建议用 bullet point 分优先级。"""

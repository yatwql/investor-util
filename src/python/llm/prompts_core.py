...
4. **成本结构**：评估成本分布、浮盈浮亏比
5. **数据质量**：评估输入数据的完整性和可靠性，结合输入中的【数据质量详细状态】段落：
   - 收盘价异常断点（停牌/零值）
   - T2/T3 降级发生频次（数据源不可用的严重程度）
   - 基金净值更新延迟——**以段落给出的交易日为基准**：净值日期等于最近交易日（T）或前一交易日（T-1）均属正常，QDII 与部分场外基金的官方净值本就是 T-1，**不构成延迟**；仅段落「净值滞后品种」中列出的品种才计延迟。**不得**用运行时刻与净值日期的自然日差判定延迟——报告可在非交易日运行，此时运行时刻与最近交易日天然相差一个自然日
   - 分红数据状态（是否可获取）
   - 个别品种数据缺失时长（连续缺失交易日数）
...
## 五、数据质量（XX/100）
评分依据（引用【数据质量详细状态】中的具体事实——降级事件与净值新鲜度基准）：
影响说明：…

...
# 架构约束代号（C1~C25）豁免文件：约束定义处正文大量引用 C1~C25（技术名称表），
...
#   CIPHER    — 架构约束代号（C1~C25，technical.md 定义）——约束定义处
...
def _doc_patterns() -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        # ── CIPHER：架构约束代号（C1~C25）──
        #  约束定义处（technical.md/llm-technical.md，CIPHER_EXEMPT_FILES）豁免；
        #  其余文档正文出现 C1~C25 属"暗号"，须改写为语义描述（原子写入/会话缓存/
...
# 所有文档正文（含 plan.md/changelog.md 等历史/计划记录）均不得出现 C1~C25）
...
def scan_file(fpath: Path, verbose: bool, chapter_only: bool = False) -> list[tuple[int, str, str, str]]:
    ...
    出现 C1~C25——约束代号属"暗号"，与历史痕迹/章节暗号不同层，处处禁。
    """

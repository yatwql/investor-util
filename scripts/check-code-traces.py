...
退出码：
  ...
  2 — 发现任务编号/章节编号引用（CODE/IDENT/CHAPTER/ROUND），应从注释/标识符中移除
      ...
      ROUND：注释中用"第 N 轮"/"轮N"指代开发迭代轮次，属迭代痕迹，
      须改用语义描述）
  3 — 仅 LOW 级别痕迹，建议人工复核
"""
...
PATTERNS: list[tuple[str, str, str]] = [
    ...
    (r"拆分自\s*[`\w._\-()]+", "HIGH", "拆分自XX（函数/模块拆分来源叙述）"),
    ...
    #  即失效，须改用语义章节名（「X」章）。章节数量/序数表述（共 N 章、
    #  合并后 N 章基线、减至 N 章、N→M 章、出现第 N 章、引号内"N 章"）由
    #  _chapter_excludes() 豁免。负向前瞻 (?!节) 排除"章节"一词；
    #  [1-9]\d? 限 1~99，不匹配中文数字"一章"、小节号"4.2"（数字后非"章"）。
    (r"第?\s*[1-9]\d?\s*章(?!节)", "CHAPTER", "章节编号引用（N 章/第 N 章，须用语义章节名）"),
    #
    # ═══ ROUND：迭代轮次暗号（用"第 N 轮"/"轮N"指代开发迭代轮次） ═══
    #  "第 N 轮" / "轮N" 是开发迭代痕迹（计划分轮实施，如"第 4 轮"、"轮5 落地"、
    #  "轮13 验收标准"），代码注释中不应记录迭代轮次，须改用语义描述。
    #  轮次数量/序数表述（共 N 轮、目标 N 轮、N 轮每轮、轮询、轮动等运行时/
    #  计数词汇）由 _round_excludes() 豁免；`[1-9]\d?` 限 1~99，
    #  不匹配中文数字"一轮"（一轮行情是业务表述，非迭代轮次）。
    (r"第\s*[1-9]\d?\s*轮", "ROUND", "迭代轮次引用（第 N 轮，属开发迭代痕迹）"),
    (r"[1-9]\d?\s*轮", "ROUND", "迭代轮次引用（N 轮，属开发迭代痕迹）"),
    (r"轮[1-9]\d?\b", "ROUND", "迭代轮次引用（轮N，属开发迭代痕迹）"),
    #
    # ═══ VERSION：版本号 / 发布 / 迭代标记 ═══

...
PATTERNS: list[tuple[str, str, str]] = [
    ...
    #   架构约束代号（C1~C21，technical.md 定义）属"暗号"——注释应直接描述当前
    ...
    #   与 Excel 单元格（A1/B2/F4）结构性冲突，故注释侧不捕获 C1~C21 之外的裸
    #   "族字母+数字"——F4/B6 作为**标识符**由 IDENTIFIER_PATTERNS 捕获（见下）。
    #   注释侧仅捕获无歧义的系列代号形状（_series/系列）与 C1~C24 约束代号。
    #
    ...
    #  架构约束代号（C1~C24）：按约束表实际编号精确匹配，避免误伤十六进制色值
    #  （C00000）、C25+ 等；前限非 ASCII 字母/数字，避免 AB14/MC19 内嵌命中。
...
EXCLUDE_LINE: list[str] = [
    ...
    #  不误伤""十六进制色值（C0 非 1~24）""超出 C1~C24 范围"等字面量描述豁免边界

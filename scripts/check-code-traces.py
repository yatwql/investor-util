...
PATTERNS: list[tuple[str, str, str]] = [
    ...
    #   架构约束代号（C1~C25，technical.md 定义）属"暗号"——注释应直接描述当前
    ...
    #   与 Excel 单元格（A1/B2/F4）结构性冲突，故注释侧不捕获 C1~C25 之外的裸
    #   "族字母+数字"——F4/B6 作为**标识符**由 IDENTIFIER_PATTERNS 捕获（见下）。
    #   注释侧仅捕获无歧义的系列代号形状（_series/系列）与 C1~C25 约束代号。
    #
    ...
    #  架构约束代号（C1~C25）：按约束表实际编号精确匹配，避免误伤十六进制色值
    #  （C00000）、C26+ 等；前限非 ASCII 字母/数字，避免 AB14/MC19 内嵌命中。
    #  约束表新增编号时必须同步放开此处范围，否则新约束代号成为检测盲区。
    (
        r"(?<![A-Za-z0-9])C(?:[1-9]|1[0-9]|2[0-5])\b",
        "CODE",
        "架构约束代号（C1~C25，须用语义描述替代，如原子写入/会话缓存/数据契约）",
    ),
    #
    # ═══ MAGIC：字母+数字 魔法编号 ═══
    ...
    (
        r"(?<![A-Za-z0-9#])[A-Z][A-Za-z]*[0-9]{1,3}(?![A-Za-z0-9])",
        "MAGIC",
        "字母+数字魔法编号（如 R11/P1/D8/HH6，须用语义名替代）",
    ),
    ...
    (
        r"(?<![A-Za-z0-9])[A-Za-z]{1,}-[0-9]{1,3}(?![0-9])",
        "DASHTASK",
        "字母-数字组合（如 F-1/TASK-22/D-8，疑似任务编号）",
    ),
    # ═══ UNDERSCORE：字母_数字 疑似无意义代码 ═══
    #  注释中"字母_数字"组合（F_1/H_1/MINE_22）疑似无意义代码标识。小写前缀
    #  （changed_1m/syl_1y 等语义短名）经 _under_excludes() 豁免。
    (
        r"(?<![A-Za-z0-9])[A-Za-z]{1,}_[0-9]{1,3}(?![0-9])",
        "UNDERSCORE",
        "字母_数字组合（如 F_1/MINE_22，疑似无意义代码标识）",
    ),
    #
...
EXCLUDE_LINE: list[str] = [
    ...
    #  不误伤""十六进制色值（C0 非 1~25）""超出 C1~C25 范围"等字面量描述豁免边界

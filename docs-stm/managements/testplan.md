...
创建日期：2026-06-26
最后更新：2026-07-09（v0.3.2 — 数据降级重构：新增 provider_registry 测试覆盖 + 文档同步）
...
| **S14: LLM 不启用** | — | TUI 不按 L，直接生成报告 | 菜单 E / H / B 等（无 L） | 核心报告完整生成；无 LLM API 用量页签（Excel 无页签 16、HTML 无第 16 节）；LLM 分析章节整体不出现 |

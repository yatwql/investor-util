...
| **S19: news_correlation 启用 + 5 模块混合** | `enabled_llm.news_correlation=true`，2 缓存 + 2 成功 + 1 新闻关联成功 | 菜单 L | LLM API 用量页签明细表含 5 行（含新闻关联分析(LLM)）、状态正确（橙/绿/绿/绿/绿）、费用计算含所有 5 模块、总 Token 为 5 模块之和 |

...
| `tui/tui_menu.py` | 所有 15 选项 | 合法/非法输入、Ctrl+C、空目录选择、多文件导航 |
...
| **S14: LLM 不启用** | — | `test_llm_disabled.py` | TUI 不按 L，直接生成报告 | 菜单 E / B 等（无 L） | 核心报告完整生成；无 LLM API 用量页签（Excel 无页签 18、HTML 无第 18 节）；LLM 分析章节整体不出现 |

...
> 文档版本：0.10.1-dev
> **编号源**：`rf-next = 205`（新增问题取此编号，完成后更新为 +1；已用最大 rf-204，递增保证唯一，归档不回收。若与历史归档冲突，运行 `scripts/check-task-numbering.py` 校验）

---

## 当前待处理问题

### P1 — 版本一致性检查缺陷（2026-08-04）

| # | 问题 | 修复方向 |
|---|------|----------|
| **rf-204** | `check-version-consistency.py` `_check_contains` 仅判断文本是否**包含**目标版本串，可能被正文偶然出现的版本号误导而漏检头部版本号行。实测：v0.10.0 发布时 review-findings.md 头部 `文档版本：0.9.13-dev` 未更新，但因正文 rf-114 修复方向提到 `v0.10.0` 被误判 [OK] | 改为校验「文档头部版本行」精确匹配（限定 `文档版本：{v}` 行首），而非全文 contains；`--fix` 模式可自动修正头部版本行 |

...
| P3-13 | **Debate 幻觉过滤误杀** | `llm/generators.py` `_filter_hallucinated_codes` | — | 正则 `[A-Za-z0-9]{4,6}` 将 HTML/CSS 标签（`color`、`size`、`font`、`style`、`strong`、`flash`）和品类名（`QDII`）误判为虚构品种代码，过滤 3768→0 字符，白脸/红脸生成失败，整体回退普通 expert_review，导致 debate 机制形同虚设。 | **中**：静默降级核心 LLM 分析质量，多角度辩论机制失效 | 为过滤加白名单（常见英文词豁免），或改用仅筛选纯数字代码模式 |

# 投资复盘助手 - 自我审查问题记录
> 文档版本：0.10.16-dev
> **编号源**：`rf-next = 297`（新增问题取此编号，完成后更新为 +1；已用最大 rf-296，递增保证唯一，归档不回收。若与历史归档冲突，运行 `scripts/check-task-numbering.py` 校验）
...
#### P2C — LLM 数据质量口径缺陷（2026-09-04 借鉴分析发现）

> health_check「数据质量」维度与 expert_review 降级摘要口径可能自相矛盾。

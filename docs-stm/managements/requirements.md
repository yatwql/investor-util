...
创建日期：2026-06-26
最后更新：2026-07-10（v0.3.5 — Provider Chain 熔断升级 + LLM 模块拆分 + 测试模式优化）
...
### 4.1 DataSourceRegistry 数据源注册中心（v0.3.2+）
...
3. **系统数据缓存统计**（v0.2.85+）— 底部追加当前会话的数据缓存命中/未命中/总请求数/命中率，仅在有缓存请求时显示。
...
| 9 | 年均股息率 | 历史分红数据。† `YYYYE` 为 `datetime.now().year` 动态生成，跨年自动更新 |
| 10 | 来源明细 | 来自哪些基金的持仓 |

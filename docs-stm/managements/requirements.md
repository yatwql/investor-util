...
| L | 生成全系列完整版报告(Excel+HTML) | 同时生成 HTML + Excel，含新闻、模块 7-8 LLM 增补内容（模块 1-8） |
| C | 配置持仓信息目录 | 配置持仓文件的存放目录 |
| F | 配置持仓信息文件名 | 配置持仓文件的文件名 |
| R | 配置报告输出目录 | 配置报告文件的输出目录（默认 reports） |
| 1 | 更新基础类缓存 | 主动更新基金业绩/持仓/基准/新闻缓存（含 news_ 新闻缓存 + llm_news_corr LLM 新闻关联分析缓存） |
...
缓存统一存放在 `data/cache/` 目录，采用 JSON 格式，由 `src/cache.py` 提供泛用键值对存储接口。缓存分为三个层级，各有不同的生命周期和刷新策略。
...
- **指纹的构成**：
  - 全球政经局势：A 股/美股指数行情 + 持仓汇总（总市值/总盈亏/分类）
  - 智囊团深度复盘：持仓详细价格/盈亏数据
...
1. **持仓文件发生变更**（新增品种/清仓/修改份额）：
   ...
   - 自动清除关联的 `fund_benchmarks.json`
   - 新增资产的 `price_*`、`fund_perf_*`、`fund_hold_*` 自动预热（避免首次取价延迟）
   - 更新 `holdings_tracking.json` 中的指纹和代码集合
2. **LLM 缓存指纹失效**：
   - 全球政经局势：指数行情或持仓汇总数据变化后，指纹与旧缓存不匹配
   - 智囊团深度复盘：任一持仓的价格/份额/盈亏数据变化后，指纹自动失效
   - 下次生成 L 菜单时自然使用新数据
3. **菜单 [2] 主动清除**：
...
| 全球政经局势（LLM） | `llm_macro` | 14400 秒（4 小时） | `llm_global_macro_{fingerprint}.json` |
...
| 菜单 | 功能 | 清除范围 |
|---|---|---|
| `[1] 更新基础类缓存` | 主动刷新基金业绩排名、持仓明细、业绩基准、新闻及新闻 LLM 关联分析 | `fund_perf_*`、`fund_hold_*`、`fund_benchmarks.json`、`news_*`、`llm_news_corr_*` |
...
- **行业**（`电力`）— 匹配持仓/穿透之外的新闻热词

富化后关键词按 持仓→穿透→行业 顺序排列，在 Excel 中以格式化字符串显示，在 HTML 中以不同颜色标签区分（持仓→蓝色、穿透→紫色、行业→灰色）。

...
把「此源需要什么凭据」**声明在数据里**，缺失时给出「缺什么 → 去哪申请」的可读指引，而不是等运行期拿到 HTTP 401 再当作「源不可达」反复重试。**当前全部数据源免费无需凭据，声明表为空**——机制由注入合成声明的单元测试证明可用，不为演示把假数据源写进生产注册表。

| 需求标识 | 需求描述 |
|:---------|:---------|
| R-CRD-01 | **声明即数据**：`CredentialSpec`（`source_id` / `display_name` / `env_var` / `apply_url` / `note`）登记进 `CREDENTIAL_SPECS`；接入需 key 的源时补一行 `register_credential_spec(...)` 即可，不新造第二套配置体系 |
| R-CRD-02 | **就绪判定**：读环境变量判就绪，**空白串（含纯空白/换行）视为缺失**（防「设了空值以为配好了」）；源未声明 → 不需要凭据 |
| R-CRD-03 | **可读指引**：缺失时给出「缺少凭据（数据源：X）——请设置环境变量 VAR；申请地址：URL」，措辞与既有 LLM 凭据缺失指引同风格 |
...
| R-CRD-06 | **体检分组**：`doctor` 增设「数据源凭据」组——无声明报「N 个数据源均无需凭据（免费源）」，存在缺失则报失败项并附变量名修复建议；「数据源」组过滤掉凭据跳过项，避免同一问题被两个组重复计为失败 |
| R-CRD-07 | **凭据值安全**：只从环境变量读取，**值永不落日志、永不写入报告与缓存**——日志与就绪矩阵只出现「变量名 + 是否就绪」及申请地址 |

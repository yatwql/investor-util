...
### 5.7 数据源适配契约（实验功能 `datasource_adapter`）
...
| R-ADP-06 | **开关关闭时行为逐字节不变**：链路返回既有手写映射对象本身；开启后改用适配器映射 |
...
| R-ADP-08 | 实验开关 `datasource_adapter` 默认关闭，三面上屏同源注册表（TUI 菜单 `[S]` / Web 配置面板 / CLI `--experiment`） |

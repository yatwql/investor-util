...
| R-WEB-08 | 页面提供配置编辑面板，覆盖 TUI 可编辑配置全集（路径/报告章节/子模块/匿名化/对比指数池/LLM 开关/辩论实验）：修改即写共享配置文件（config.json / llm_settings.json / features.json）并写前自动备份 `.bak`，TUI/CLI 下次读取即生效 |

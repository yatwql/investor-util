...
| `signal_pre_digest` | bool | true（默认开启） | 信号预消化启停：市场温度/估值分位/尾部风险预消化为带方向标注的信号行（`信号：… 看多/看空/中性/风险高/中/低`）注入专家复盘与持仓体检提示词。**默认开启**——读侧注入既有信号，不额外调用 LLM、不写盘；无可用信号时静默跳过、缓存键逐字节不变 |

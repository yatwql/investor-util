...
| R-LLM-DB-PROCON-06 | Token 预算守卫：基于字符数计量保护 token 消耗。实际中文+Markdown 输出约 1 字符 ≈ 1 token，pro+con 累计字符超过 `int(max_total_tokens_per_report)`（默认 48000）时触发 1× 超限→跳过 synthesis 阶段（返回 pro+con 拼接）；超过 2× 超限时跳过全部 debate（回退标准模式）。配合 `per_call_max_tokens` 单段输出上限（默认 12288，经 `max_tokens_override` 生效）双闸保护，预算覆盖三段式真实成本避免 synthesis 频繁被砍 |
...
| 键 | 类型 | 默认值 | 说明 |
|:---|:----:|:------:|:-----|
| `debate.procon.per_call_max_tokens` | int/null | 12288 | 正反辩论每阶段 max_tokens 覆盖（null=按代码兜底 12288；2026-09-16 由 8192 上调 50%，因智囊团复盘三段式下 pro 段实测被 8192 截断），经 `max_tokens_override` 优先于模块级 `max_tokens_expert_review` |

...
| 1 | 更新基础类缓存（含基金业绩/持仓/经理/基准等） | 主动更新基金业绩/持仓/基准/行业分类/新闻/盈利预测/行业资金流向/分红/基金经理/持仓重合度缓存（含 fund_perf_*、fund_hold_*、fund_benchmarks.json、industry_*、news_*、llm_news_item_*、profit_forecast_*、sector_flow_*、dividend_*、fund_manager_*、fund_overlap_*）。纯股票持仓（无基金）时自动跳过基金项，仍主动重拉行业分类、分红、盈利预测、资金流向 |
...
> **收市后价格缓存新鲜度验证**：盘后即使长 TTL 未到期，`fetcher/price.py` 仍会校验缓存中 `price_date` 是否为当前交易日。若盘中因 Tencent 名称校验降级到 EastMoney 写入了上一交易日净值，盘后重新请求时将识别该残留数据，自动清除缓存并强制从 Tencent 重取，确保收市后首次报告生成时价格已更新。
...
基于持仓个股的市值和 PE 数据进行加权投票，判定基金在六宫格风格箱中的位置（大盘/中盘/小盘 × 成长/价值/混合）。三级降级链路：push2（精确）→ Tencent 扩展字段（可靠）→ 代码前缀估算（兜底）。与历史快照比较计算网格曼哈顿距离漂移评分（0-4 分三级：轻度/中度/严重）。详见 §8.2.9。

> **以下为新闻与预警模块**（菜单 B/L，依赖新闻数据）：

#### 财经新闻热点与持仓关联分析（Excel + HTML）

关键词来源（直接持仓名称/代码 + 穿透TOP10 资产 + 东方财富三级行业/概念板块），5 源并行获取，子串匹配 → 按匹配关键词数降序 → TOP N（`news_top_count` 配置，默认 300）。

> **召回保障**：每个新闻源的原始获取量 = `max(500, news_top_count × 2)`，确保去重后仍有充足候选条目供关联排序。华尔街见闻 API 硬上限 100 条除外。
...
| 8 | 预测EPS(2026E)† | 机构盈利预测 |
| 9 | 年均股息率 | 历史分红数据。† `2026E` 为当前预测年份，代码中硬编码不含动态推断，2027 年后需手动更新 |
| 10 | 来源明细 | 来自哪些基金的持仓 |
...
> *空数据降级：基金经理 API 不可用时该模块不隐藏，显示灰色占位文本，页签保持可见。*
...
> *空数据降级：持仓数据不足或无重叠基金时该模块不隐藏，显示灰色占位文本。*
...
> *空数据降级：持仓集中度数据不可用时（如基金无持仓穿透数据）该模块不隐藏，显示灰色占位文本。*
...
> *空数据降级：push2/Tencent 双链路均不可用且无历史快照时该模块不隐藏，显示灰色占位文本。*
...
| push2 数据源熔断 | 行业分类/概念板块/基金风格分析自动降级为备用数据或代码估算结果 | 连续 3 次失败后本运行周期跳过 push2（`eastmoney_industry.py`），走备用链路或兜底方案 |

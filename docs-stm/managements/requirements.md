...
详见项目根目录 [README.md](../README.md) 的「目录结构」章节。
...
| 基金业绩排名 | 天天基金 `fund.eastmoney.com/pingzhongdata/{code}.js`（JS 变量解析） | — |
...
缓存统一存放在 `data/cache/` 目录，采用 JSON 格式，由 `src/cache.py` 提供泛用键值对存储接口。缓存分为两层：

- **单条缓存**：由 `src/fetcher.py` 在首次取数时自动创建，按 `{类型前缀}_{代码}.json` 命名规则。缓存命中时无需再调 API。
- **合并缓存**：通过 TUI 菜单 [1]/[2] 手动刷新时生成的全量快照。价格和基金业绩数据**不**写入合并文件，直接以单条形式存储。
...
| 文件名 | 使用目的 | 缓存刷新 | 生成方式 |
|---|---|---|---|
| `fund_benchmarks.json` | 业绩比较基准对照表——每只基金的比较基准名称 | 30 天 | 自动 + [1] 刷新 |
| `portfolio_latest.json` | 持仓主数据快照——含最新价/净值/市值/账户等全部字段 | 持仓更新时 | [2] 更新持仓缓存 |
| `penetration_cache.json` | 资产穿透 TOP10 计算结果——合并后的底层标的及来源明细 | 每日 | [2] 更新持仓缓存 |

#### 单条缓存文件（引擎自动管理）

命名规则：`{类型前缀}_{证券代码}.json`
...
| `news_*.json` | **财经新闻数据**（三大源的新闻去重合并结果），按日期/关键词缓存 | 24 小时 |
...
| 持仓数据 | `hold` | 604800 秒（7 天） | `fund_hold_{code}.json`、`portfolio_latest.json`、`penetration_cache.json` |
| 财经新闻 | `news` | 86400 秒（24 小时） | `news_*.json` |
| 基准数据 | `benchmark` | 2592000 秒（30 天） | `fund_benchmarks.json` |

### 6.5 手动刷新

TUI 菜单 [1] 刷新基金业绩/持仓/基准缓存；菜单 [2] 刷新价格/指数/穿透缓存。

### 6.6 降级规则

缓存过期但 API 请求失败时使用最近 7 天内的过期缓存数据；缓存文件损坏时自动删除并触发重新获取。详细降级设计见 [plan.md](plan.md)。
...
7 列明细：排名 | 名称 | 代码 | 穿透市值 | 占比 | 板块 | 来源明细。
...
- **HTML**：自动渲染在报告第 6 节（来源标注为"新浪财经"/"东方财富"/"财联社"）

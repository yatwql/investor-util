...
### 5.8 数据源凭据就绪指引（实验功能 `datasource_credential_ready`）
...
| R-CRD-08 | 实验开关 `datasource_credential_ready` 默认关闭，**关闭时不产生任何行为分支**（链路照常尝试、无跳过项、无就绪行、无凭据组，输出与未引入本机制时逐字节一致）；三面上屏同源注册表（TUI 菜单 `[S]` / Web 配置面板 / CLI `--experiment`） |

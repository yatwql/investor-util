...
|:----:|:-----|:------|
| **报告生成** | | |
| E | 生成基础版 Excel 分析报告 | 投资分析汇总、市值核算明细表、持仓分类表、资产穿透TOP10、基金业绩分析、组合演进、数据源可用性矩阵（6 个始终显示 + 组合演进默认开启，由 `enable_portfolio_evolution` 控制；行动建议由 `enable_action` 控制，默认关；不含基金深度分析/新闻/历史走势） |

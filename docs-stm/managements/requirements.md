...
创建日期：2026-06-26
最后更新：2026-07-14（v0.5.0——归档 v0.4.x 变更记录）
...
| B | 生成全系列报告(Excel+HTML) [按板块配置] | 同时生成 HTML + Excel，含新闻（如有）、B 系列基金深度分析（如有，基金经理变更监控/持仓重合度矩阵/持仓集中度监控/基金风格分析）、历史走势（如有），不含 LLM 增补内容。各板块按 [P] 菜单配置决定是否包含 |
| L | 生成全系列完整版报告(Excel+HTML) [含LLM，按板块配置] | 同时生成 HTML + Excel，含新闻、智能预警、B 系列基金深度分析、历史走势、LLM 增补内容。各板块按 [P] 菜单配置决定是否包含 |
...
| P | 配置报告板块可见性（B系列/新闻/历史走势） | 交互切换 B 系列基金深度分析、新闻与预警、历史走势 3 个板块的启用/停用，关闭后对应页签完全隐藏 |
...
| 页签 | 触发菜单 | 功能概要 | 字段详情 |
|:-----|:--------:|:---------|:--------:|
| 投资分析汇总 | E/B/L | 指数行情 + 账户汇总（日期、A股/美股指数、总市值/成本/盈亏） | §8.2.1 |
| 市值核算明细表 | E/B/L | 15 列持仓明细（含分账户小计+总计），数据复用穿透 + 价格/净值模块 | §8.2.2 |
| 持仓分类表 | E/B/L | 按资产属性+投资分类双维度分组，10 列明细，复用市值核算结果 | §8.2.3 |
| 资产穿透TOP10 | E/B/L | 基金底层标的拆解→合并→排序，底部标注剩余市值及无法穿透基金 | §8.2.4 |
| 基金业绩分析 | E/B/L | 同类排名 + 业绩基准对比，依赖天天基金 API 排名/收益率数据 | §8.2.5 |
...
| 组合历史走势 | E/B/L | as-if 市值曲线、累计收益率、最大回撤、年化波动率（数据不可用时显示占位） | §8.2.12 |
| 回撤分析 | E/B/L | 回撤面积图、最大回撤值/率/区间 | §8.2.13 |
...
>
> **F 系列**（组合历史走势、回撤分析）在 E/B/L 全菜单模式下均可用，数据可用性由 `history.analysis` 配置（`"off"` / `"prompt"` / `"auto"`）控制，详情见 [§8.2.12](#8212-组合历史走势)。F1 持仓快照对比不受此配置影响，始终自动执行（详见 [§8.4](#84-持仓快照f1)）。
...
|:----:|:---------|:--------:|:--------:|:-----|
| 1 | 投资分析汇总 | E/B/L | — | 指数行情、账户汇总、持仓概况 |
| 2 | 市值核算明细表 | E/B/L | — | 15 列明细，含分账户小计+总计 |
| 3 | 持仓分类表 | E/B/L | — | 按资产属性+投资分类分组聚合 |
| 4 | 资产穿透TOP10 | E/B/L | — | 基金底层标的拆解合并排序 |
| 5 | 基金业绩分析 | E/B/L | — | 同类排名、收益率、基准对比 |
...
| 16 | 组合历史走势 | E/B/L | — | as-if 市值曲线、累计收益率、最大回撤、年化波动率 |
| 17 | 回撤分析 | E/B/L | — | 回撤面积图、最大回撤值/率/区间 |
...
> **页脚附带 F1 快照对比摘要：** 回撤分析页脚嵌入持仓快照对比卡片（详见 §8.5 F1 vs F2 对比），展示本次与上次报告之间的总市值变化、总盈亏变化及持仓变动 TOP5。
...
| `default_menu_key` | str | `"L"` | 手动 | TUI 菜单默认选项快捷键，取值为任一有效菜单键（如 E/B/L），启动后光标自动定位 |
...
| **_板块可见性** | | | | 以下字段控制报告板块启停（菜单 [P] 配置） |
| `enable_b_series` | bool | `true` | P | B 系列基金深度分析（#6 基金经理变更监控、#7 持仓重合度矩阵、#8 持仓集中度监控、#9 基金风格分析）。`false` 时对应页签完全隐藏 |
| `enable_news` | bool | `true` | P | 新闻与预警（#10 财经新闻热点与持仓关联分析、#11 智能预警）。`false` 时对应页签完全隐藏。注：此为板块级别开关，与 `news_sources`（数据源级别）层级不同 |
| `enable_history` | bool | `true` | P | 历史走势（#16 组合历史走势、#17 历史回撤分析）。`false` 时对应页签完全隐藏。F1 持仓快照不受此影响，始终执行 |

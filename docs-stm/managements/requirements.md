...
创建日期：2026-06-26
最后更新：2026-07-02（v0.2.59 — 补充指数双链路、LLM API 用量章节、业绩评价 4→5 级、数据源/缓存菜单描述对齐）
...
1. **基础评级** — 基于同类排名百分位：≤10%=优秀、10%~30%=良好、30%~50%=稳定、50%~75%=偏差、>75%=较差
2. **超额收益修正** — 超额收益评分≥80 上调一级、40~80 维持、<40 下调一级
3. **显示与标色** — 优秀=红色、良好=默认、稳定=蓝色、偏差=绿色、较差=深绿
...
#### LLM API 用量（Excel + HTML — 用量统计，v0.2.15+）
...
LLM 分析章节（全球政经局势、智囊团深度复盘、持仓体检报告、穿透深度分析）以及**已启用 LLM 服务支持的财经新闻热点与持仓关联分析**（`llm_settings.json` 中 `enabled_llm.news_correlation = true`），每次调用 LLM API 或命中缓存时，用量数据均自动累计到会话级计数器。LLM API 用量页签/章节将这些数据汇总展示，不作为独立 LLM 调用模块存在。

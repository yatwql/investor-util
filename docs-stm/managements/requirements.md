...
| R-FIN-09 | **报告消费章**：新增「财务指标」独立章（Excel 页签 + HTML partial + 导航分组），开关 功能开关 `financial_indicator` 默认关；数据驱动（无数据 → 章节隐藏/写占位），不改变既有章节输出 |
...
| 9 | 风格与因子分析 | B/L | 基金深度分析 | 一章三区块：市值/PE 加权六宫格+漂移评分 + 风格因子回归（β 系数+基准对照+显著性）+ 行业 Beta 子表（功能开关 `industry_beta` 开关，默认关） |
...
#### 6.4.20 成本流水（功能开关 `cost_lots` 子模块，默认关）

开关 功能开关 `cost_lots` 开启后，汇总/市值/分类页签渲染成本分档 + 资金加权收益率（XIRR）+ 分红累计：
...
| 需求标识 | 需求描述 |
|:---------|:---------|
| R-FRD-01 | 新增独立报告章节「持仓个股财报摘要」，开关 功能开关 `financial_report_digest` 默认关；仅覆盖 A 股（SSE/SZSE/BSE），按沪深京代码前缀映射 FMP 风格符号 |

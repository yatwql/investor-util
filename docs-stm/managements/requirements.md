...
| 6 | 持仓关系矩阵 | B/L | 基金深度分析 | 一章两区块：重合度（Jaccard+重叠率双指标热力图）+ 相关性（下三角 Pearson 热力图+显著性标注） |
| 7 | 持仓集中度监控 | B/L | 基金深度分析 | top3/5/10 占比+环比变化+三级预警 |
| 8 | 风格与因子分析 | B/L | 基金深度分析 | 一章三区块：市值/PE 加权六宫格+漂移评分 + 风格因子回归（β 系数+基准对照+显著性）+ 行业 Beta 子表（功能开关 `industry_beta` 开关，默认关） |
| 9 | 行动建议 | E/B/L | 行动建议（enable_action 控制，**默认开**） | 再平衡信号/交易纪律/调仓建议/收益归因 |
| 10 | 财经新闻热点与持仓关联分析 | B/L | 新闻 | 5 源新闻关键词匹配，可选 LLM 增强 |
| 11 | 全球政经局势 | L | LLM | 基于指数+持仓结构生成 |
| 12 | 智囊团深度复盘 | L | LLM | 三阶段圆桌会议；Feature Flag 开启辩论模式（正反辩论/条件推理/集中度问答）时输出含辩论内容并标注"🧪 辩论模式"/"🧪 实验模式"标签 |
...
| 17 | 数据源可用性矩阵 | E/B/L | 基础报表 | 各数据源实时可用性状态汇总 |
| 18 | LLM API 用量 | L | LLM | 当前会话的 LLM API 用量汇总（强制末位） |
...
#### 6.4.19 成本流水（功能开关 `cost_lots` 子模块，默认关）

...
| 16 | 持仓基本面 | E/B/L | 基础报表 | 同页签两区块：①财务指标（开关 `financial_indicator`，需数据底座就绪）②持仓个股财报摘要（开关 `financial_report_digest`，需 DataSinking key）；两开关各控一块，任一就绪即显示 |
...
| # | 区块 | 说明 |
|---|------|------|
| 1 | 区块① 财务指标 | 字段定义与规则见 §5.9（R-FIN-*）与 §6.4 转载说明；Excel 19 列（名称/代码/报告期/文种/营收/归母净利/营收同比/净利同比/毛利率/ROE/负债率/经营现金流/每股收益/每股净资产/PE/PB/质量档/年度趋势/来源） |
| 2 | 区块② 持仓个股财报摘要 | 字段定义与规则见 §6.12（R-FRD-*）；Excel 9 列（名称/代码/报告期/文种/标题/披露日/摘要/来源/原文链接） |
...
| 需求标识 | 需求描述 |
|:---------|:---------|
| R-FRD-01 | 「持仓基本面」章节的**区块② 持仓个股财报摘要**，功能开关 `financial_report_digest` 默认关；仅覆盖 A 股（SSE/SZSE/BSE），按沪深京代码前缀映射 FMP 风格符号 |
...
| R-FRD-03 | 对持仓 + 穿透中的 A 股标的，取**最新报告期**（**跨文种**：半年报/季报通常比年报新，故一次索引请求取最近若干篇后按报告期倒序取最新一篇；`datasink.doc_types` 非空时作文种白名单，默认为空=不限）的正文摘要：先取该文档**实际章节名清单**（`/documents/{id}/sections`，带缓存），再按 `datasink.sections` 偏好顺序做**子串匹配**取精确章节名（默认偏好：管理层讨论与分析 → 经营情况讨论与分析 → 主要财务数据 → 主要会计数据 → 董事会报告，季报自动落到财务数据章节；清单不可得时回退偏好名直取），命中者拼接后按 `datasink.max_chars` 截断。首选项章节不可用（404）时**继续试下一候选**，不据此判定整只标的「未取到财报」 |

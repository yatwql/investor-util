...
| R-FIN-13 | **数据底座门禁**：财务指标章需 `datasink.enabled` 开启且凭据就绪（见 R-VAL-04）；未就绪时返回 None（章节隐藏、不写占位），且**门禁先于任何取数**（零网络开销） |
...
| R-FIN-09 | **报告消费章**：新增「财务指标」独立章（Excel 页签 + HTML partial + 导航分组），功能开关 `financial_indicator` 默认关；数据驱动（无数据 → 章节隐藏/写占位），不改变既有章节输出 |
...
| R-VAL-04 | **数据底座门禁**：财务指标章与真实历史估值分位**以 DataSinking 数据底座就绪为前提**（配置位 `datasink.enabled` 开启 **且** 凭据已配置）；判定为**纯本地检查**（零网络请求、零延迟）。未就绪时两者**静默回原样**——财务指标章整体隐藏（不写占位）、估值列文案与免责语逐字保持引入前形态，不干扰阅读与业务分析 |
...
| 4 | 基金业绩分析 | E/B/L | 基础报表 | 同类排名、收益率、基准对比 |
| 5 | 基金经理变更监控 | B/L | 基金深度分析 | 快照式变更检测（1/3/6月窗口），三级预警 |
| 6 | 持仓结构与集中度 | B/L | 基金深度分析 | 一章三区块：重合度（Jaccard+重叠率双指标热力图）+ 相关性（下三角 Pearson 热力图+显著性标注）+ 集中度（top3/5/10 占比+环比变化+三级预警） |
| 7 | 风格与因子分析 | B/L | 基金深度分析 | 一章三区块：市值/PE 加权六宫格+漂移评分 + 风格因子回归（β 系数+基准对照+显著性）+ 行业 Beta 子表（功能开关 `industry_beta` 开关，默认关） |
| 8 | 行动建议 | E/B/L | 行动建议（enable_action 控制，**默认开**） | 再平衡信号/交易纪律/调仓建议/收益归因 |
| 9 | 财经新闻热点与持仓关联分析 | B/L | 新闻 | 5 源新闻关键词匹配，可选 LLM 增强 |
| 10 | 全球政经局势 | L | LLM | 基于指数+持仓结构生成 |
| 11 | 智囊团深度复盘 | L | LLM | 三阶段圆桌会议；Feature Flag 开启辩论模式（正反辩论/条件推理/集中度问答）时输出含辩论内容并标注"🧪 辩论模式"/"🧪 实验模式"标签 |
| 12 | 持仓体检报告 | L | LLM | 五维度量化评分 |
| 13 | 穿透深度分析 | L | LLM | 行业集中度+国别暴露 |
| 14 | 组合历史走势与回撤 | B/L | 历史 | 一章两区块：走势表（as-if 市值曲线+累计收益率+最大回撤+年化波动率+基准对比）+ 回撤矩阵（回撤面积图+独立回撤事件明细）+ 危机区间标注（2015/2018/2020/2022） |
| 15 | 组合演进 | E/B/L | 演进 | 多快照趋势追踪：总市值/总盈亏/HHI/TOP 持仓占比变迁，快照不足时占位 |
| 16 | 数据源可用性矩阵 | E/B/L | 基础报表 | 各数据源实时可用性状态汇总 |
...
### 6.12 持仓个股财报摘要（DataSinking 全文本财报）

| 需求标识 | 需求描述 |
|:---------|:---------|
| R-FRD-01 | 新增独立报告章节「持仓个股财报摘要」，功能开关 `financial_report_digest` 默认关；仅覆盖 A 股（SSE/SZSE/BSE），按沪深京代码前缀映射 FMP 风格符号 |

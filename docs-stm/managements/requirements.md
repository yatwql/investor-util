...
|:----:|:-----|:------|
| **报告生成** | | |
| E | 生成基础版 Excel 分析报告 | 投资分析汇总、市值核算明细表、持仓分类表、资产穿透TOP10、基金业绩分析、组合演进、数据源可用性矩阵（6 个始终显示 + 组合演进默认开启，由 `enable_portfolio_evolution` 控制；行动建议由 `enable_action` 控制，默认开启；不含基金深度分析/新闻/历史走势） |
...
| R-WEB-08 | 页面提供配置编辑面板，覆盖 TUI 可编辑配置全集（路径/报告章节/子模块/匿名化/对比指数池/LLM 开关/功能开关——后者界面上分「实验性功能」与「常规开关」两块）：修改即写共享配置文件（config.json / llm_settings.json / features.json）并写前自动备份 `.bak`，TUI/CLI 下次读取即生效 |
...
| R-FIN-09 | **报告消费章**：新增「财务指标」独立章（Excel 页签 + HTML partial + 导航分组），开关 `report_submodules.financial_indicator` 默认关；数据驱动（无数据 → 章节隐藏/写占位），不改变既有章节输出 |
...
| 7 | 持仓关系矩阵 | B/L | 基金深度分析 | 一章两区块：重合度（Jaccard+重叠率双指标热力图）+ 相关性（下三角 Pearson 热力图+显著性标注） |
| 8 | 持仓集中度监控 | B/L | 基金深度分析 | top3/5/10 占比+环比变化+三级预警 |
| 9 | 风格与因子分析 | B/L | 基金深度分析 | 一章三区块：市值/PE 加权六宫格+漂移评分 + 风格因子回归（β 系数+基准对照+显著性）+ 行业 Beta 子表（`report_submodules.industry_beta` 开关，默认关） |
| 10 | 行动建议 | E/B/L | 行动建议（enable_action 控制，**默认开**） | 再平衡信号/交易纪律/调仓建议/收益归因 |
...
| 16 | 组合历史走势与回撤 | B/L | 历史 | 一章两区块：走势表（as-if 市值曲线+累计收益率+最大回撤+年化波动率+基准对比）+ 回撤矩阵（回撤面积图+独立回撤事件明细）+ 危机区间标注（2015/2018/2020/2022） |
| 17 | 组合演进 | E/B/L | 演进 | 多快照趋势追踪：总市值/总盈亏/HHI/TOP 持仓占比变迁，快照不足时占位 |
...
**区块三 行业 Beta 子表**（`report_submodules.industry_beta` 开关，默认关）：
...
#### 6.4.20 成本流水（`report_submodules.cost_lots` 子模块，默认关）

开关 `report_submodules.cost_lots` 开启后，汇总/市值/分类页签渲染成本分档 + 资金加权收益率（XIRR）+ 分红累计：
...
| `report_submodules` | dict | `data_quality` 与 `market_temperature` 默认开，其余 6 项默认关 | — | 报告子模块开关：`data_quality`（数据质量仪表盘，长期可信核心默认开）/ `industry_beta`（行业 Beta 子表）/ `candidate_compare`（候选基金比较）/ `cost_lots`（成本流水）/ `valuation_percentile`（估值分位）/ `market_temperature`（市场温度，**默认开**：无数据时该行静默省略）/ `financial_report_digest`（持仓个股财报摘要）/ `financial_indicator`（财务指标），后 7 项默认关 |

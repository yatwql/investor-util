...
**TTL 优先级链（按优先级从高到低）：**
1. `llm.json` 中的 `cache_ttl_macro` / `cache_ttl_expert`（仅限 LLM 缓存）
...
| 菜单 | 功能 | 清除范围 |
|---|---|---|
| `[1] 更新基础类缓存` | 主动刷新基金业绩排名、持仓明细、业绩基准 | `fund_perf_*`、`fund_hold_*`、`fund_benchmarks.json` |

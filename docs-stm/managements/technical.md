...
**Excel 渲染**：合并章 `portfolio_history_drawdown` 页签分「走势表 + 回撤矩阵」两区块——走势表每基准一列（归一化值）+ 指标汇总矩阵（累计收益/最大回撤/年化波动率/起止日，仅一份），回撤矩阵为独立回撤事件明细（含恢复耗时），危机区间标注为 2015/2018/2020/2022 静态日期表 + 区间统计，尾部风险统计为 VaR(95/99)/最大单日跌幅/最长连续下跌/最大跌幅后恢复五行（百分比按 FMT_PERCENT 存小数，样本不足写「样本不足」占位）。旧 `portfolio_history`/`drawdown_analysis` 两个独立页签已物理合并删除。

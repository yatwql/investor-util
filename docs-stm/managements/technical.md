...
`f_context` 遵循 C19 Schema 契约：所有键必须在 `docs-stm/plan/better-investment-advice/data-channels-schema.md` 中预定义类型、版本号和写入/消费模块后，才能在代码中使用该键。
...
| **C19** | **f_context Schema 契约** — 所有 f_context 键必须先在 docs-stm/plan/better-investment-advice/data-channels-schema.md 中定义类型、版本号、写入/消费模块后，才能在代码中新增该键的使用 | 无 schema 定义的键在管线中类型不匹配时引发难调试的 KeyError，且多人并行开发时互相不知道对方新增的键 | 违反时集成测试不通过 | report/orchestrator.py、所有向 f_context 注入数据的模块 |

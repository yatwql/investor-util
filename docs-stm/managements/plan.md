...
- **`cache.py` 大函数拆分**：`_is_market_open`（75 行）提取多策略判断函数，`check_and_refresh_caches`（68 行）指纹计算与缓存清除分离，`set()`（57 行）gzip 判断逻辑提取。
- **测试覆盖增补**：`llm/api.py`（熔断/重试/回退/截断）、`report/excel_generator.py`（18 个页签写入）补充独立单元测试。

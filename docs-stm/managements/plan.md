...
| ② | **plan-7** 因子 probe 验证 | P2 | 0.5d | 零成本决策闸门：提前验证 CSI 风格指数数据源，排除不可行项 |
...
> **plan-1**～**plan-7**（plan-4 已放弃）合计 ~21d 预排。
...
| plan-7 | **因子暴露分析（推荐② probe 先行）** | 中证因子代理 + OLS 回归 + 风格归属饼图 | 3.5d | ⚠️ **实施前先做 0.5d probe**：验证 CSI 风格指数（000919/000920/000922/000925/000930）在 Sina 可用性；≥3 个有效 → MVP 3 因子，否则按 plan-4 模式放弃 |

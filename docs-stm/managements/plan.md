# 投资复盘助手 — 实现计划
> 文档版本：0.11.1-dev
> **编号源**：`plan-next = 54`（新增计划项取此编号，完成后更新为 +1；已用最大 plan-53，递增保证唯一，归档不回收。若与历史归档冲突，运行 `scripts/check-task-numbering.py` 校验）

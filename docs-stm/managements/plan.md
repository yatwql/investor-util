...
| plan-7 | **因子暴露分析（推荐② MVP 3 因子）** | 中证因子代理 + OLS 回归 + 风格归属柱状图 | 2.5d | ✅ **probe 已完成（2026-08-01）**：Tencent 4/5 主候选有效且新鲜，300成长（sh000920）停更由 500成长/低波替代 → 按 MVP 3 因子实施（详 `plan-advanced-analysis.md §4`，含架构约束遵从表与技术债预置；Sina 备用链路 404 见 rf-103） |

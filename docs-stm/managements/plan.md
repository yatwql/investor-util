...
| ② | **plan-7** 因子暴露分析（MVP 3 因子） | P2 | 2.5d | ✅ **已完成**（2026-08-02，迭代 v0.9.5）：OLS 回归 + 风格归属 + 停更剔除 + C19 契约 + HTML/Excel 渲染（详 §plan-7 条目）；probe 已验证（2026-08-01）MVP 3 因子可行；300成长停更由 500成长替代 |
...
| plan-7 | **因子暴露分析（推荐② MVP 3 因子）** | 中证因子代理 + OLS 回归 + 风格归属柱状图 | 2.5d | ✅ **已完成（2026-08-02）**：`analysis/factor_exposure.py`（OLS 回归/样本下限/停更剔除/LOCF）+ `report/orchestrator.py` 编排注入 + C19 13 键契约 + HTML MODULE 17 柱状图 + Excel 页签 + 单元 11 例/场景 5 例（详 `plan-advanced-analysis.md §4`，含架构约束遵从表与技术债预置）；**实施前技术债 rf-102/103/104/106 已全部处理**（Tencent 钳位 2000 + 解析容错、Sina 降级接受、因子替代、days 语义澄清） |

...
| 序号 | 任务 | 状态 | 估时 | 阻塞 | 说明 |
|:----:|------|:----:|:----:|:----:|------|
| 56 | **LLM 事实锚定校验器** | ✅ 纯算法层完成 | 24h | — | 纯算法层（数值一致/品种存在性/排名正确）已实现并集成到 orchestrator，报告末尾追加校验摘要；LLM 增强层待评估 |
...
| 69 | **缓存雪崩随机 TTL 修复** | 🆕 | 4h | — | 同类缓存 TTL 增加 ±15% 随机偏移，偏移种子基于 key 哈希 |
| 70 | **metrics 测试用例（edge 场景）** | 🆕 | 8h | ←指标计算 | 全 None/空列表/极端值/不足 20 日→置信度 low，`@pytest.mark.edge` 隔离 |

...
- **资产穿透TOP10 基金全值误入排名**（#O15）：`_merge_fund_layer` 中当 `fetch_fund_holdings` 返回空数据时，fallback 将基金本身按全值加入 `merged`，导致基金名称（如"财通成长优选混合A"）出现在穿透 TOP10 中且穿透市值等于基金全值而非底层持仓按比例分配值。修复：取不到持仓数据的基金不进 `merged`，仅记入 `unknown_mv` 并在页脚提示。另补充回归测试 2 项。

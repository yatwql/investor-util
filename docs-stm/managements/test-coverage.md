...
| `scenario` | ~20s | ~3min |
| `regression` | ~20s | ~3min |
| `verify,regression` | ~34s（verify+regression 顺序之和） | ~4min（verify+regression 顺序之和） |
| `dev-verify` | ~25s | ~2min |
| `verify` | ~13s | ~46s |
...
| **配置管理** | `config/`, `core/constants.py` | `unit/config/test_config*.py` + `test_features.py`（功能开关名解析与取数 + 开关注册表活性：每项开关须有消费者 / 陈旧开关不得回归 / 无消费者键加载时告警 + 开关注册表统一不变量：单条声明五字段齐备 / 分组覆盖全集不重不漏 / 默认值遵循分组 / 默认值字典为派生投影） | 371 |

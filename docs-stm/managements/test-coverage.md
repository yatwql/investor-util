...
> 注：以下统计为 `def test_` 函数级计数（不含参数化展开）。`all` 模式全量 4746 项（2026-08-04 实时收集快照，`scripts/collect-test-coverage.py` 生成，需在项目 `.venv` 环境运行以包含 pandas 依赖的测试文件）。
...
| **配置管理** | `config/`, `core/constants.py` | `unit/config/test_config*.py` | 244 |
...
| ├─ `unit_report` | 报表生成（Excel/HTML 各页签写入、基金深度分析模块、风格与因子分析合并章（风格表 + 因子回归 + 行业 Beta 子表）、数据降级/占位/可用性矩阵、调仓 What-if/组合演进/行动建议双端呈现、数据质量仪表盘、尾部风险接线、Chart.js 图表数据构建/裁剪、暗色模式 theme.js） | 1377 |
| ├─ `unit_config` | 配置管理（config/llm_settings/llm_key；含报告序号配置校验、enable_action 等板层开关） | 244 |

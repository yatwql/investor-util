...
| 标记 | 覆盖模块 | 覆盖项数 |
|:-------|:---------|:--------:|
| `unit`（父标记） | 12 子组合计 | **6607** |
| ├─ `unit_providers` | 数据源 Provider（腾讯/东方财富/天天基金等，含 push2 估值字段 PE/PB 提取 + 行业数据 REST 接口、非有限数值归一防线 safe_num 拦截） | 279 |
| ├─ `unit_fetcher` | 数据获取调度（价格/指数/基金/行业/API 异常/熔断预检/冷却恢复/链路失败原因采集 FailureDiagnostics/数据源适配契约三段式与行情域三源等价性/估值字段网关（Provider Chain 路由 + push2 会话复用与按代码去重）） | 353 |

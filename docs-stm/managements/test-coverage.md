...
> 注：以下统计为 `def test_` 函数级计数（不含参数化展开）。`all` 模式全量 4623 项（2026-08-04 实时收集快照，`scripts/collect-test-coverage.py` 生成，需在项目 `.venv` 环境运行以包含 pandas 依赖的测试文件）。
...
| **分析计算** | `analysis/`(liquidity, rebalance, fx_exposure, bond_yield, alignment_correction, drawdown_warning, drawdown_events, factor_exposure, correlation, portfolio_evolution, action_advisor, trade_discipline, rebalance_advisor, whatif, whatif_backtest) | `unit/analysis/test_{liquidity,rebalance,bond_yield,fx_exposure,alignment_correction,drawdown_warning,drawdown_events,factor_exposure}*.py` + `test_correlation.py` + `test_correlation_edge.py` + `test_portfolio_evolution.py` + `test_action_advisor.py` + `test_trade_discipline.py` + `test_rebalance_advisor.py` + `test_whatif.py` + `test_whatif_backtest.py` + `test_whatif_backtest_edge.py` | 416 |
...
| ├─ `unit_report` | 报表生成（Excel/HTML 各页签写入、基金深度分析模块、数据降级/占位/可用性矩阵、调仓 What-if/组合演进/行动建议双端呈现、数据质量仪表盘、Chart.js 图表数据构建/裁剪、暗色模式 theme.js） | 1343 |
...
| ├─ `unit_analysis` | 分析计算（流动性/再平衡/汇率/口径修正/回撤预警/回撤事件/因子暴露/相关性矩阵/组合演进/行动建议/交易纪律/调仓建议可行化层/调仓 What-if 时序回测） | 416 |

...
| ├─ `unit_fetcher` | 数据获取调度（价格/指数/基金/行业/API 异常/熔断预检/冷却恢复/链路失败原因采集 FailureDiagnostics/数据源适配契约三段式与行情域三源等价性/估值字段网关（Provider Chain 路由 + push2 会话复用与按代码去重）/财务指标链路（主源→解析支路降级）+ 财务指标章装配/页签/管线契约台账） | 397 |

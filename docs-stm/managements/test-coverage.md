...
> 注：以下统计为 `def test_` 函数级计数（不含参数化展开）。`all` 模式全量 3786 项（2026-07-30 快照，已清理重复穿透场景文件 + 冗余缓存单体文件）。
...
| 标记 | 覆盖模块 | 覆盖项数 |
|:-------|:---------|:--------:|
| `unit`（父标记） | 11 子组合计 | **3474** |
| ├─ `unit_providers` | 数据源 Provider（腾讯/东方财富/天天基金等） | 191 |
| ├─ `unit_fetcher` | 数据获取调度（价格/指数/基金/行业/API 异常/熔断预检/冷却恢复） | 263 |
| ├─ `unit_llm` | LLM 模块（API 路由/熔断/指纹/骨架/prompts/generators/llm_content 写入/Token 成本跟踪/降级回退） | 630 |
...
| 标记 | 覆盖范围 | 覆盖项数 |
|:-------|:---------|:--------:|
| `llm` | 全部 LLM 相关（unit_llm 630 + scenario_llm 32，其中 487 项标有 llm 标记），**全部为 mock 测试，无需真实 API key** | **487** |

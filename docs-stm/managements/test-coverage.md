...
<!-- env-table:start -->
| 环境属性 | dragonball（2026-09-18 实测） | stallman-NB1（2026-08-06 实测） |
|:---------|:---------------------------|:---|
| 操作系统 | Linux | Windows |
...
| 并行级别 | medium | medium |
| worker 数 | 8 | 4 |
| 采集日期 | 2026-09-18 | 2026-08-06 |
<!-- env-table:end -->

#### 各模式耗时对照

<!-- duration-table:start -->
| `--mode` | dragonball（2026-09-18 实测） | stallman-NB1（2026-08-06 实测） |
|:---------|:---------------------------:|:---:|
| `unit` | ~18s | ~4min |
| `standard` | ~20s | ~4min |
| `scenario` | ~25s | ~3min |
| `regression` | ~25s | ~3min |
| `verify,regression` | ~42s（verify+regression 顺序之和） | ~4min（verify+regression 顺序之和） |
| `dev-verify` | ~28s | ~2min |
| `verify` | ~17s | ~46s |
...
| ├─ `unit_scripts` | scripts/ 工程脚本（历史痕迹检查工具自检/豁免/补强模式/回归场景元描述豁免/多行 docstring 提取/任务编号一致性检查/语义命名索引双向校验/3 类暗号匹配自检/机器信息采集与耗时表格渲染/环境耗时对照文档自动更新/失败用例提取 data-jsonblob 回归） | **358** |
...
| 标记 | 覆盖范围 | 覆盖项数 |
|:-------|:---------|:--------:|
| `llm` | 全部 LLM 相关（带 `llm` 跨类标记），**全部为 mock 测试，无需真实 API key** | **757**（2026-09-20 复核：收集快照 757） |

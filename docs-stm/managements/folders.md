...
| **源代码合计** | — | **338** | **90,389** | 主程序 + 模板 + 脚本 + SVG |
| **测试代码** | Python | **399** | **118,349** | `src/test/` 所有 `.py` 文件（含近期 ruff format 全仓重排） |
| **测试用例** | — | — | **7,885 个** | `pytest --collect-only` 统计（`scripts/collect-test-coverage.py` 实时收集快照，不含 opt-in live 套件） |
| **用户文档** | Markdown | **11** | **5,437** | 含 README.md（207 行）；行数为 README + manuals 之和 |
| ├ manuals/ | 用户手册分册 | 10 | 5,230 | 配置/faq/快速上手/TUI/CLI/Web 三种模式指南等 |
| **项目文档** | Markdown | **145** | **55,749** | 含 CLAUDE.md（75 行）；md 口径（CLAUDE.md 1 + managements 10 + plan 2 + archive md 132），py/txt 不计行 |

...
| **源代码合计** | — | **311** | **84,307** | 主程序 + 模板 + 脚本 + SVG |
| **测试代码** | Python | **379** | **112,135** | `src/test/` 所有 `.py` 文件（含近期 ruff format 全仓重排） |
| **测试用例** | — | — | **7,395 个** | `pytest --collect-only` 统计（`scripts/collect-test-coverage.py` 实时收集快照，不含 opt-in live 套件） |
| **用户文档** | Markdown | **11** | **5,184** | 含 README.md（205 行）；行数为 README + manuals 之和 |

...
| 辅助脚本 | Python | 32 | 9,451 | `scripts/`（启动脚本 + CLI 命令行包装、测试驱动、工具检查、任务编号检查、性能测试、LLM 幻觉率评估、测试覆盖计数、代码/文档历史痕迹检查、语义命名索引校验、Claude Code hook 安装/校验、Web 冒烟脚本、push2 连通性诊断、SVG 架构图检查） |
| **源代码合计** | — | **331** | **89,168** | 主程序 + 模板 + 脚本 + SVG |
| **测试代码** | Python | **395** | **116,636** | `src/test/` 所有 `.py` 文件（含近期 ruff format 全仓重排） |
...
| **项目文档** | Markdown | **145** | **55,301** | 含 CLAUDE.md（75 行）；md 口径（CLAUDE.md 1 + managements 10 + plan 2 + archive md 132），py/txt 不计行 |
| ├ managements/ | 管理文档 | 10 | 10,511 | 变更日志/目录树/测试计划/技术设计/开发者指南等 |
| ├ archive/ | 版本归档 | 132 | 44,348 | 各版本 changelog/plan/review-findings 与设计文档归档（132 md 43,759 行） |

...
| **测试代码** | Python | **372** | **109,295** | `src/test/` 所有 `.py` 文件（含近期 ruff format 全仓重排） |
| **测试用例** | — | — | **7,218 个** | `pytest --collect-only` 统计（`scripts/collect-test-coverage.py` 实时收集快照，不含 opt-in live 套件） |
...
| ├ managements/ | 管理文档 | 10 | 10,085 | 变更日志/目录树/测试计划/技术设计/开发者指南等 |

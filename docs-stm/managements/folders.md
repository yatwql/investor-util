...
| **测试代码** | Python | **373** | **108,709** | `src/test/` 所有 `.py` 文件（含近期 ruff format 全仓重排） |
...
| **项目文档** | Markdown | **138** | **52,188** | 含 CLAUDE.md（75 行）；md 口径（managements 10 + plan 2 + archive 121 md），py/txt 不计行 |
| ├ managements/ | 管理文档 | 10 | 9,911 | 变更日志/目录树/测试计划/技术设计/开发者指南等 |
| ├ archive/ | 版本归档 | 129 | 42,429 | 各版本 changelog/plan/review-findings 等（125 md 41,971 行 + 3 py 446 行 + 1 txt 12 行） |
| ├ plan/ | 中间设计文件 | 2 | 223 | **当前无在办设计文档**（已实现的设计文档随迭代归档：DataSinking 财报摘要与基本面数据源/财务指标两份现位于 v0.10.x 归档的 `datasink-financial-report-digest/` 与 `financial-indicator-source/`；功能开关注册表统一与联接基金穿透两份位于 `feature-switch-registry/` 与 `feeder-fund-penetration/`；外部借鉴系列 14 份位于 `tradingagents-borrowing/`、`augur-borrowing/`、`openbb-borrowing/`、`llm-fingerprint-prompt-coverage/`） |
...
│   │   │   └── archived_plan.0.11.x.md    # plan-44 完成态记录
...
│   └── plan/                          #   中间设计文件（在办设计文档）
│       ├── section-consolidation-design.md # 报告章节整合设计（设计层：四项合并/可见性模型扩展/约束对照）
│       └── section-consolidation-iteration.md # 报告章节整合实施层（迭代施工单：命名统一/接缝地图/逐批步骤）
│
├── CLAUDE.md                         # AI 编程助手指引

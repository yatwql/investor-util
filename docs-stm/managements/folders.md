...
| **源代码合计** | — | **314** | **85,257** | 主程序 + 模板 + 脚本 + SVG |
| **测试代码** | Python | **386** | **113,367** | `src/test/` 所有 `.py` 文件（含近期 ruff format 全仓重排） |
| **测试用例** | — | — | **7,486 个** | `pytest --collect-only` 统计（`scripts/collect-test-coverage.py` 实时收集快照，不含 opt-in live 套件） |

...
> **对齐 plan.md（MVP 3 因子，2.5 天）**。probe 已完成，数据源可行性已验证，剩余为接入/计算/输出三阶段（比原 5 因子设计少动量/低波 2 个因子 + 图表降级为柱状图）。
...
| **plan-1**（交互式 HTML） | 图表框架迁移 | 已约定：plan-7 图表回退 Canvas 柱状图，**不阻塞在 plan-1**（§计算方案图表输出） |

...
不创建独立的 `history.py`/`HistoryFetcher` 类。改为在现有 Provider 中新增历史数据函数，通过 Provider Chain 路由：

| 新增函数 | 所在文件 | 用途 | 所属 chain |
|:---------|:---------|:-----|:-----------|
| `get_stock_history(code, days)` | `providers/tencent.py` | 主链路（腾讯 K 线） | `history_stock` |
| `get_stock_history(code, days)` | `providers/sina.py` | 备用链路（新浪 K 线） | `history_stock` |
| `get_fund_history(code)` | `providers/tiantian.py` | 唯一链路（天天基金净值） | `history_fund_otc` |
...
_DEFAULT_CHAINS: dict[str, list[str]] = {
    # ... 现有 ...
    "history_stock": ["tencent_history", "sina_history"],
    "history_fund_otc": ["tiantian_history"],
}
```

**Provider 注册要点：**

- `register_provider()` 的 `tier` 参数为 **int**（非 string）。建议 `tencent_history` 手调 tier=2（与 tencent 实时同源），`sina_history`/`tiantian_history` 默认 tier=3
- `tencent` 和 `tencent_history` 是不同的 provider name，熔断器独立，互不干扰
- `register_default_chains()` 在 `chain.py` import 时自动调用；修改 `_DEFAULT_CHAINS` 后需重启应用或显式调用注册。测试代码中需调用 `register_default_chains()` 确保链注册可用

**代码类型路由（C1 约束）：**

OTC 基金代码通过排除法识别（非 A 股、非 ETF 前缀的 6 位数字代码）。
...
> **说明**：`is_fund_code()` 不新增到 `code_utils` 中——C1 约束要求 code_utils 只提供底层原语，组合判断在业务层（Calculator 内部）实现，遵循 `classify_holdings`（`market_value.py`）的既有模式。
...
`_fetch_with_fallback()` 是 all-or-nothing 单值获取模式（检查缓存→无缓存→遍历 provider→成功则缓存全部结果），历史数据需要 get-or-fetch-merge-store 增量合并——两者不兼容。历史数据改为 Provider 函数自身实现 **cache-aware 增量合并**：
...
def get_stock_history(code: str, days: int = 30) -> list[dict]:
    ...
    # 3) 仅拉取增量
    new_data = _fetch_tencent_kline_range(code, start_from=last_cached_date)
...
| F2 历史 K 线 | `cache/` 子包，Provider 侧增量合并 | 7 天 | C2 |
| F2 历史净值 | `cache/` 子包，Provider 侧增量合并 | 30 天 | C2 |
...
|:-----------|:---------|:----------|:-------------|
| `history_stock` | Tencent/Sina | **T2**（同实时价格） | 始终可见（失败→占位文本） |
| `history_fund_otc` | Tiantian | **T3**（同实时净值） | 始终可见（失败→占位文本） |
...
```python
# excel_generator.py / html_writer.py 中 type="history" 的处理：
if section["type"] == "history":
    visible = True  # 始终可见，数据状态由 _data_status 控制
```
...
🔹 **验收**：save/load/list/prune 5 场景；空目录返回 None。  
⏪ 删除文件。
...
### R3：LLM 注入 + 流程嵌入（F1 完成）

**修改**：
- `handlers_report.py`：`_cmd_generate_full()` 中嵌入快照读写 + 差异注入
- `generators.py`：`DiffSummary → prep`
- `prompts.py`：差异感知提示词（"相比上次报告"等）

**指纹去重**：`fingerprint` 比对，无实际变化时跳过差异段落生成

🔹 **验收**：首次无差异，第二次有"相比上次"；损坏快照不崩溃；Δ 列出现在 Excel。  
...
**修改**：
- `providers/tencent.py` — 新增 `get_stock_history(code, days)`，端点 `web.ifzq.gtimg.cn/appstock/app/fqkline/get`
- `providers/sina.py` — 新增 `get_stock_history(code, days)`，端点 `money.finance.sina.com.cn/getKLineData`
- `providers/tiantian.py` — 新增 `get_fund_history(code)`，复用现有 `pingzhongdata/{code}.js` JS 解析模式

**缓存模式**：Provider 侧增量合并（§4.3），非 `_fetch_with_fallback()` 的全量模式。

**约束**：C5（`make_http_client()`）

🔹 **验收**：mock 3 个端点正确解析；空数据返回 `[]`；净值字段缺失时静默降级。  
⏪ `git checkout` 恢复 3 文件。

---

### R5：Chain 注册 + PortfolioHistoryCalculator + 缓存增量合并

**修改**：
- `chain.py`：注册 `history_stock` / `history_fund_otc` chains（§4.1 注册验证注意事项见上方说明）
- `provider_registry.py`：注册 provider（`tencent_history`/`sina_history` 建议手调 tier=2）
- `registry.py`：新增 DataModuleDef

**新文件**：`report/portfolio_history.py`

- `PortfolioHistoryCalculator` — 遍历持仓 → 按代码类型路由（A 股/ETF → history_stock，OTC → history_fund_otc，exclusion-based）→ 并行获取 → 计算回撤/波动率/收益率
- 缓存合并逻辑不在 Calculator 中，在 Provider 函数内部（§4.3 方案）

**约束**：C1（code_utils 组合逻辑）+ C4（session_cache）+ C6（register_default_chains 时序注意）

**测试**：`test_portfolio_history.py`（unit_report）+ `test_history_chain_edge.py`（edge, C12）

🔹 **验收**：回撤计算误差 <0.01%；volatility 边界处理正确；chain 注册可用；缓存合并正确（增量拉取→合并→去重）。  
...
**修改**：
- `handlers_report.py`：F2 触发逻辑（§3.2 二阶段询问 + history.analysis 配置解析）+ 全量重渲染模式（§3.2 评审要点）
- `excel_generator.py`：新增 type="history" section 渲染分支（始终可见，数据状态由 _data_status 控制）
- `html_writer.py`：新增走势/回撤章节（全量重渲染，不追加 HTML）
- `registry.py`：新增 `_REPORT_SECTION_DEFAULT` + `_REPORT_SHEET_NAMES`
- `data_status.py`：新增 §5.3 的 3 条 STATUS_MESSAGES

**约束**：C7（registry 注册）+ C14（render context 传递）
...
🔹 **验收**：
- `history.analysis: "prompt"` → 报告完成后询问，选 Y 生成走势，选 N 跳过
- `history.analysis: "auto"` → 缓存全命中时自动附加
...
| TD-2 | **无 pandas 手写时间序列** | 架构约束 | R7 | 时间对齐/缺失填充手写，保持零依赖 | 永不（架构决策） |
| TD-3 | **LLM 提示膨胀** | 性能 | R3+R8 | 差异+趋势数据 ~2-3k tokens | R8 后评估，>20% 则裁剪 |
| TD-4 | **历史数据无质量检查** | 运维 | R4-R6 | 0 值/未来日期不告警 | R13 或独立脚本 |
| TD-5 | **指数比较锁死沪深300** | 灵活性 | R12 | config 硬编码 | 用户要求多基准时扩展 |
| TD-7 | **报表离线无图** | 功能局限 | R6+R9 | Chart.js 依赖 CDN | FAQ 说明，可选内联 |
...
| # | 风险 | 概率 | 影响 | 控制措施 |
|:--|:-----|:-----|:-----|:---------|
| R1 | **Tencent/Sina 历史 API 变更** | 中 | F2 不可用 | Provider Chain 双链路 + 过期缓存降级；F1 不受影响 ✅ |
| R2 | **15s+ 耗时增加** | 高 | 体验下降 | 二阶段触发（不强制）；进度条反馈 ✅ |
| R3 | **LLM token 增加** | 中 | 成本上升 | `trim(top_n=5)` 裁剪差异数据；F1 指纹去重避免无变化时生成 ✅ |
| R4 | **历史熔断误阻断实时价格** | 低 | 实时行情被波及 | 独立 provider name（`tencent_history` ≠ `tencent`），熔断器隔离已验证 ✅ |
| R5 | **用户删除了 snapshots 目录** | 低 | F1 无对比 | 退化为 `is_first_check`，下次生成时重建 ✅ |
| R6 | **F2 section 显示逻辑遗漏** | 中 | 报告无走势章节 | C7 注册 + `type="history"` 分支 + `section_visible_dict` 强制可见 |
| R7 | **Provider 侧缓存增量合并测试覆盖不足** | 中 | 合并逻辑缺陷 | 专用测试用例如 `test_history_chache_merge.py`，覆盖增量/全量/空/重复 4 种场景 |
| R8 | **`tencent_history` tier 级别与预期不一致** | 低 | 审计报告混淆 | 实施时显式调用 `register_provider("tencent_history", tier=2)` |
...
| C5 HTTP 客户端统一 | 是 — F2 所有 HTTP 请求 | ✅ `make_http_client()` |
| C6 Provider Chain 必经 | 是 — F2 历史数据 | ✅ `history_stock/fund_otc` chain 注册到 registry。Provider 侧增量缓存合并的 API 调用路径直调 Provider 函数，熔断器/名称注册仍通过 registry 管理 |
...
| C14 渲染期数据不可写入模块级全局变量 | 是 — F2 走势数据 | ✅ render context 传递 |

...
|:--|:--:|:--|:--|
| ① 景气方向 / 通胀属性 | 25 | 穿透后重仓（含基金底层）按**板块**聚合市值占比，命中配置的 `boom_keywords` 的板块计「景气市值占比」分档给分；命中 `defensive_keywords` 的占比作反向扣减 | 无需外部数据（名称/板块即可算） |
...
| ③ 全球视野 / 中国比较优势 | 15 | 命中 `global_edge_keywords` 的板块市值占比分档 + QDII/港股等**境外资产占比**加分（全球暴露） | 无需外部数据 |

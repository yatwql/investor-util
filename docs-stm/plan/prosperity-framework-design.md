...
**口径修订记录（真实持仓复核）**：原实现「有穿透数据就只看穿透（覆盖仅 34.6% 市值）」，且 `benchmarks` 结构假设错误、同一标的可同时计入景气与防御。已修订为：**并集覆盖 + 归一**（把未被穿透覆盖的持仓按板块纳入）、基准兼容 `list[dict]`、景气/防御**互斥归类**；覆盖率在证据中披露（如「覆盖 48.47% 市值」），读者可知评分的分母。
...
|:--|:--:|:--|:--|
| ① 景气方向 / 通胀属性 | 25 | **并集口径 + 归一**：穿透重仓各底层标的（含基金拆解）+ 未被穿透覆盖的其余直接持仓（QDII/联接/债基等，按 `classify_sector` 板块）合并后按已覆盖市值归一；命中 `boom_keywords` 的权重按 60% 满分档折算，命中 `defensive_keywords` 的权重反向扣减（**同一标的按防御优先归类，互斥不重复计**） | 无需外部数据（名称/板块即可算） |
...
| ③ 全球视野 / 中国比较优势 | 15 | 命中 `global_edge_keywords` 的权重（口径同维度①并集归一，40% 满分档）+ QDII/港股等**境外资产占比**加分（全球暴露） | 无需外部数据 |

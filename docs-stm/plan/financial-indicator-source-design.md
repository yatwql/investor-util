...
- `datasink_indicator` 是一层**解析适配器**：输入为已缓存的 `financial_report` 全文，输出为标准指标字段，不新增对 DataSinking 的请求（避免重复占用配额）。
...
按优先级取用（`/documents/{id}?section=`）：

1. `主要会计数据与财务指标`（多数市场年报/半年报的标准章节，直接含多期指标表）
2. `财务报告`（附注所在章节，用于补齐现金流/负债细项）
3. 季报以 `主要财务数据` 章节为准（季报指标表更精简）

章节标题用**现有 `datasink.sections` 配置**扩展，不新增第二份清单。

### 5.2 解析步骤

| 步 | 处理 |
|---|---|
| 1 | Markdown 表格识别（表头 + 数据行） |
| 2 | 指标名归一（同义标题映射到标准字段，如「营业收入」/「营业总收入」） |
| 3 | 单位归一（读 `> 单位：元` / `万元` / `亿元` / `%`，统一到「元」与小数比例） |
| 4 | 多期对齐（本期 / 上期 / 同比列；报告期以元数据 `report_period` 为准，不从正文猜） |
| 5 | 缺失容错（解析不到的字段取 `None`，不猜、不填 0） |

**关键纪律**：解析器只做「数字与单位」，不做口径推断；「归母 / 合并 / 单季」等口径差异以表格标题为准并在 `source` 备注，避免把不同口径混入同一序列。
...
| `datasink_indicator` | DataSinking 指标解析 | 从全文解析指标的备用支路（provider 名） |
| `financial_indicator_extract` | 财务指标提取 | Markdown 表 → 标准指标的解析模块 |
...
| 文件 | 改动 |
|:--|:--|
| `src/python/fetcher/financial_indicator.py` | 新增。指标域取数编排（akshare 全量 → 按代码过滤；解析支路调度） |
| `src/python/providers/akshare_financial.py` | 新增。akshare 财务指标封装（超时保护、字段命名） |
| `src/python/analysis/financial_indicator_extract.py` | 新增。Markdown 表 → 标准指标的纯解析层 |
...
| 层 | 用例 |
|:--|:--|
| 解析层 | 表头同义归一、单位换算（元/万元/亿元/%）、多期对齐、字段缺失取 `None`、合成 Markdown 样本 |
| 适配器 | 三段式契约自检、alias/defaults 指向真实字段、输出恰为标准字段集 |
| 链路 | akshare 失败落解析支路、两者皆失败返回空、过期缓存兜底、凭据/接口异常不计熔断 |
...
| 全文表格口径差异导致数字误读 | 只取标准章节、口径以表标题为准、解析不到取 `None` |

...
### PRE-01: Rf 东方财富 datacenter API 稳定性测试
- **估时**: 8h（1 天）
- **文件**: 无代码改动，纯手工测试
- **阻塞**: 否（独立）
- **依赖**: 无
- **描述**: 验证 `datacenter-web.eastmoney.com RPTBOND_BOND_YIELD_AND_SPREAD` 接口的长期可用性：(1) 连续请求 50 次确认不限频；(2) 确认返回数据结构稳定（YIELD 字段名、类型、单位不跳变）；(3) **新增 24 小时持续限频测试**：每小时请求 1 次（模拟生产频率），连续 24 次确认不限频；(4) **新增 7 天连续采样**：验证备用源 `worldgovernmentbonds.com` HTML 结构连续 7 天不变；(5) 记录测试结果到 `docs-stm/tmp/rf-test-report.md`。结果决定 P1-01 是主用 API 还是直接走用户配置。⚠ **决策影响**：PRE-01 结果直接影响 Phase 1 Rf 基础设施路线——若 API 不可用，整个 Phase 1 Rf 模块（P1-01~P1-03，28h）降级为纯手动配置，夏普/Beta/Alpha 全部不可计算，波及 5 个阶段约 50h 开发工作。因此 PRE-01 测试完成后**必须在 bond_yield.py 顶部注释中嵌入决策日期和决策结论**，并在 PRE-01-D 中记录影响范围通知。
...
- **阻塞**: 否
- **依赖**: PRE-01
- **描述**: 根据 PRE-01 测试结果进行决策分支：(1) API 可用且稳定 → 按计划执行 P1-01（东方财富 API，12h）、P1-02（备用源，8h）、P1-15（Rf 测试，8h），合计 28h；(2) API 不可用/限频严重 → 跳过 P1-01/P1-02，直接激活 P1-03 手动配置兜底（估时缩减至 2h，因只需配置入口无需 fetcher 开发），释放的 28h 重新分配给 Phase 1 其他任务（建议优先分配给 P1-11 功能开关 12h、P1-12 断路包装器 12h、P1-04 数据质量增强 4h）。决策结果记录到 `docs-stm/tmp/rf-test-report.md`。

### PRE-02: 偏股基金指数 885005 可用性测试
- **估时**: 4h（含东方财富+新浪双 API 测试，以及自定义基金池降级方案验证）
- **文件**: 无代码改动，纯手工测试
- **阻塞**: 否
- **依赖**: 无
- **描述**: 测试 `akshare`、东方财富基金指数 API、新浪备用链路是否能获取 885005 历史行情，每个源都做 10 次连续测试确保稳定性。不可获取则 Phase 3 竞争语境降级为沪深300+自定义基金池。执行时机：**提前到 MVP 交付后立即执行**（而非 Phase 2 收尾），以便尽早决策 P3-07 技术方案。双源一致时取平均，差异 > 1.0% 时以置信度更高的源为准并在日志中记录双源分歧。
...
- **阻塞**: 否
- **依赖**: PRE-02
- **描述**: 根据 PRE-02 结果实现对应的 prompt 分支：(1) 885005 可获取 → P3-07 实施后，竞争语境使用完整三列对比表（组合、沪深300、885005）；(2) 885005 不可获取 → 预先在 `prompts.py` 中添加降级说明段落"偏股基金指数暂不可用，以下对比仅基于沪深300"，P3-07 只实现降级说明，无需 `index.py` 扩展。无论哪种结果，prompt 分支代码需合并到 dev 供后续 Phase 3 使用。
...
- **阻塞**: 否
- **依赖**: 无
- **描述**: 当前 DegradationTracker 在 fund_performance.py/penetration_sheet.py/summary.py 中已实例化但 record() 从未被调用，也缺少聚合查询接口。新增：(1) 在 DegradationTracker 上封装 get_log() → list[dict] 方法，汇总当日所有降级记录；(2) 在 Fetcher Chain 的每个降级点（现存的 fetch_with_fallback 失败点）注入 DegradationTracker.record() 调用；(3) 确保 orchestrator.py 的 data_degradation 键仅在记录非空时聚合并注入，空列表时注入空列表而非 None（避免 LLM prompt 中的 if-empty 检查）。注：此任务为 T0-01 的前置，发现记录为零时不应阻塞报告生成，降级段落显示"今日无降级记录"。
...
- **阻塞**: 否
- **依赖**: 无
- **描述**: 在 T0-01（DegradationTracker 接线）和 P1-06-A（f_context_builder 预重构）之前，先定义当前生产管线已有 f_context 键的 Schema（Pre-Schema）。覆盖约 12 个已有键（market_data、fund_data、summary、penetration、holdings_info、portfolio_history、base_currency 等），每个键标注：类型、必选/可选标记、写入模块、消费模块、写入管线阶段。同时在 orchestrator.py 各管线阶段之间插入初始类型断言 checkpoint（assert isinstance / .get() 类型守卫），开发期捕获类型不匹配。断言仅在 `__debug__` 模式下生效，生产环境零开销。**时序要求**：必须在 T0-01 之前完成，使 T0-01 注入 data_degradation 键时已有类型校验框架。P1-21 的 Full Schema（Phase 1 新增键定义 + 全量校验扩展）在 Phase 1 补充。

### T0-01: DegradationTracker→LLM 接线
- **估时**: 4h
- **文件**: `src/python/report/orchestrator.py`（L118-132 附近注入）、`src/python/report/data_status.py`（L95 已实例化但未接）
- **阻塞**: 否
...
### T0-02: 数据质量告警注入 LLM（扩展校验项 3→6）
- **估时**: 4h
- **文件**: `src/python/llm/prompts.py`（数据质量段落）
- **阻塞**: 否
- **依赖**: T0-01（降级状态数据已进入 f_context）
- **描述**: 当前 LLM "健康检查" prompt 中只有 3 类数据质量提示（数据缺失、收益率异常、基准不一致）。扩展为 6 类：(1) 收盘价异常断点，(2) 缓存过期时间接近阈值，(3) T2/T3 降级发生频次，(4) 基金净值更新延迟，(5) 分红数据状态，(6) 个别品种数据缺失时长。每类附带当前状态摘要（正常/警告/异常）。格式化为易读的一段，注入 health_check 系统提示语。同时修改 `src/python/report/orchestrator.py` 中的 `prepare_report_data()` 确保数据质量统计字段在管线中传递。
...
- **阻塞**: 否
- **依赖**: 无
- **描述**: 已有每品种 `profit`（盈亏金额），直接计算 `profit_contribution_i = profit_i / Σ|profit_j|`。按贡献降序排列，TOP 5 品种+占比格式化一段文本。注入 `expert_review` 和 `health_check` 两个 LLM prompt 的"收益来源"小节。格式示例："收益主要由 3 只品种贡献：贵州茅台(+32.5%)、腾讯控股(+18.2%)、招商银行(+12.1%)，合计占总收益 62.8%。" 注意正负号处理（亏损品种独立列出）。
...
- **描述**: 在当前 prompt 基础上追加条件推理场景块。不需对话架构，只用 prompt 引导 LLM 输出两个情景分支：(1) 上涨情景："如果未来市场上涨 20%，建议..."；(2) 下跌情景："如果未来市场下跌 20%，建议..."。格式为 LLM 输出末尾的"情景分析"段落。不改变现有 prompt 流程。纳入 MVP 使首次交付即可看到分情景建议。
...
- **依赖**: P1-01（同一文件扩展）
- **描述**: 在 `bond_yield.py` 中增加备用数据源 `worldgovernmentbonds.com`。当东方财富 API 失败时自动降级到该源。备用源返回格式不同，需单独解析逻辑。`chain.py` 中 `"bond_yield"` 链扩展为 `["eastmoney", "worldgovernmentbonds"]`。包含：(1) 备用源请求逻辑；(2) 返回格式对齐（统一为年化收益率浮点数）；(3) 降级日志（`DegradationTracker.log()`）；(4) **双源一致性仲裁规则**：偏差 ≤ 1.0% 时取均值，偏差 > 1.0% 时优先主源并记录降级至 DegradationTracker，偏差 > 2.0% 时回退到用户手动配置。**阈值提升理由**：中美 10 年期国债基准不同（中国 10 年期国债 vs 美国 10 年期 Treasury+汇率换算），两个不同市场的国债收益率天然存在系统性偏差，0.5% 阈值过于严格会导致频繁触发降级告警。1.0% 阈值兼顾异常检测与误报控制。

### P1-03: Rf 用户手动配置兜底
- **估时**: 4h
- **文件**: `src/python/config/_config_defaults.py`（新增 `risk_free_rate` 键）
- **阻塞**: 否（可独立交付）
- **依赖**: 无（但仅当 P1-01/02 均失败才实际激活）
- **描述**: 在 `_DEFAULT_CONFIG` 中新增 `"risk_free_rate": None`（None 表示自动获取，设为浮点数如 0.015 则跳过 fetcher 直接使用）。修改 `bond_yield.py` 的入口函数：先检查配置，如有值直接返回；无值才走 API 链路。同时修改 TUI 设置菜单或 config 模板注释说明此字段含义。
...
- **⚠ K-line API 降级一致性修复**（纳入 P1-04 交付，不推给下游）：当前腾讯 K 线 API（web.ifzq.gtimg.cn）使用 `fetch_with_incremental_fallback`（过期缓存兜底），新浪 K 线（money.finance.sina.cn）使用 `fetch_with_fallback`（空列表降级）。二者在"数据过期"场景下的行为不一致——腾讯返回过期数据 + 日志告警，新浪返回空列表。日收益率管线消费的成交额和收盘价字段需要统一降级语义：`get_kline_data()` 出口处封装一层统一适配器 `_unify_kline_fallback()`，将两种后端的行为统一为"优先最新数据，过期数据仅在最新不可获取时使用"。此修复归入 P1-04 而非推给下游消费方。
...
- **描述**: LLM 请求去重依赖 `_fingerprint()` 哈希。当前 fingerprint 只覆盖基础数据字段，不等同数据变更时不会触发重新生成。在各 `_fingerprint()` 的哈希输入中增加：(1) `risk_metrics` 的 JSON 摘要；(2) `data_degradation` 状态摘要；(3) `portfolio_daily_returns[-1]`（最近一日收益率，轻量级变更检测）。字段不存在时静默跳过（向后兼容）。

### P1-10: 数据模块注册——registry.py 注册新模块
- **估时**: 8h
- **文件**: `src/python/registry.py`
- **阻塞**: 否
- **依赖**: P1-01（bond_yield 模块存在）、P1-04（日收益率管线存在，但注册本身不依赖内容）
...
- **阻塞**: 是 ← PRE-01（API 已知可访问才能写有效 mock）
- **依赖**: P1-01
- **描述**: 为 `bond_yield.py` 编写测试用例：(1) mock 东方财富 API 正常返回 YIELD=1.68 → 断言 0.0168；(2) mock 网络超时 → 断言降级到备用源；(3) mock 备用源也失败 → 断言返回 None；(4) mock 用户配置 Rf=0.02 → 断言跳过网络请求直接返回 0.02；(5) 缓存命中测试（第二次调用不触发网络请求）。标注 `@pytest.mark.unit_providers` 并注册 marker。
...
- **描述**: (1) 在 technical.md 中明确定义 analysis/ 为"业务计算层"——消费 report/ 的输出（资产分类等）做分析计算，不得反向修改 report/ 的内部逻辑；(2) 将 report/category.py 中的权益/固收/货币判定逻辑（is_bond_fund_by_name、is_otc_fund_by_name、is_money_fund_by_name 等）提取到 code_utils.py（符合 C1 中心化原则），使 analysis/ 和 report/ 均从 code_utils.py 导入判定函数，消除 analysis→report 的逆向依赖。注意：币种分类（CNY/HKD/USD）也一并提取到 code_utils.py 新增 is_hkd_denominated()、is_usd_denominated() 函数；(3) 确认所有消费点的导入路径变更（grep 所有 import category.py 的模块）并更新；(4) 在 folders.md 中更新 analysis/ 目录描述说明其分层定位；(5) 更新 registry.py _MODULE_REGISTRY 的注释说明该表仅用于有缓存的数据模块，计算模块不注册于此。
...
- **阻塞**: 是 ← P1-01（Rf fetcher） + P1-05（组合日收益率）
- **依赖**: P1-01、P1-05
...
- **描述**: 在 `metrics.py` 中编写统一入口函数 `compute_all_metrics(f_context) → dict[str, Any]`，依次调用各指标函数，组装为一个扁平字典。key 命名规范：`sharpe_ratio`、`calmar_ratio`、`hhi`、`win_rate`、`turnover_rate`、`risk_contributions`。不可计算的指标 key 值为 None。统一入口供 `orchestrator.py` 单点调用。
...
- **阻塞**: 否
- **依赖**: P2-14、P2-16
- **描述**: 在 `expert_review` prompt 结尾增加标准化行动模板，引导 LLM 输出结构化建议。模板包含：(1) 建议类型（持有/增持/减持/止盈/止损）；(2) 涉及品种；(3) 建议理由（引用前述一个或多个信号）；(4) 优先级（高/中/低）。让 LLM 做到每一条建议都有数据支撑可追溯。
...
- **阻塞**: 是（依赖链 7 步硬串行：PRE-01→P1-01→P2-01→P3-08 及 PRE-02→P3-07→P3-08，任一环节失败均导致夏普对比列显示"--"或仅剩两列）
...
- **描述**: 实现 `check_liquidity(holdings) → list[dict]`。场内品种（股票/ETF）：`市值 / 近 20 日日均成交额`，计算变现天数。输出：(1) 各品种变现天数；(2) 累计变现总天数（若需全部卖出）；(3) 标记"当日可卖出" vs "需多日卖出"。场外基金标记为 OTC 类型交由 P3-12。注入 LLM prompt："XX 品种持仓 500 万，日均成交额 1000 万，约 0.5 日可完成变现。" **降级方案**：成交额数据失败时默认假设流动性充足，不告警。
...
- **描述**: 实现 `fx_exposure(holdings) → dict`。依据上市地 + 币种字段判断：(1) 人民币计价；(2) 港币计价（港股通）；(3) 美元计价（美股/美元债）；(4) 其他。汇总各币种占比（市值加权）。LLM 输出："约 30% 资产为美元计价，人民币近期升值 / 贬值影响约 -2%。" 注意：港股通品种为港币计价但实际换汇成本是隐含的，需标注说明。注入 `expert_review` prompt。
...
- **描述**: 实现 `scenario_analysis(beta, portfolio_value) → dict`。基于组合 Beta 的线性推导：(1) 市场 -10%、-20%、-30% 三种跌幅情景下的组合预期回撤；(2) 市场 +10%、+20%、+30% 三种涨幅情景下的组合预期收益；(3) 置信区间（假设收益率正态分布，±1σ/±2σ 区间）。输出格式：表格（情景、预期变动、置信区间）。注入 LLM prompt 的"情景模拟"段落。
...
- **描述**: 实现实质性口径修正因子计算，解决 P3-09a 无法量化的三个差异：(1) 组合综合费率估算——使用品种级别费率（管理费+托管费+申赎费）加权估算组合年化费率，用于调整费前收益对比；(2) 现金剥离——从组合收益中识别并剥离货币基金/现金管理品种的贡献，使权益部分与纯权益指数可比；(3) TWR 计算——实现时间加权收益率（True Time-Weighted Return）而非简单累计对比，消除持仓变动对收益率的影响。输出修正因子字典，LLM 据此做数据级修正而非仅口头提示。如果数据不足以计算修正因子（如费率数据不可获取），回退到 P3-09a 纯说明版本并显示"费率和现金比例数据不足，对比结果仅供参考"。
...
- **阻塞**: 是 ← P1-01（Rf 就绪）+ P1-05（组合日收益率就绪）+ P2-11a（Beta 点估计就绪）
- **依赖**: P1-01、P1-05、P2-11a
...
- **描述**: 实现 `infer_risk_level(holdings) → str`。依据：(1) 权益类占比；(2) 单品种集中度；(3) 历史换手率；(4) 持仓品种波动率均值。输出 4 级：保守/稳健/进取/激进。标注"推断结果仅供参考，建议通过问卷确认"。
...
PRE-01 ─→ P1-01 ─→ P1-02 ─→ P1-10        (API 可用分支)
  │       │               └→ P1-15
  ├→ PRE-01-D (决策门: API 不可用→直接 P1-03)
  └→ P1-03 (兜底, 估时缩减至 2h)
...
P1-04 ─→ P1-05 ─→ P2-01(P1-01+Rf), P2-02    (组合日收益率→夏普前置+卡玛)
  │              │
...
P2-01(P1-01+P1-05) → P2-08 → P2-09 → P2-14 → P2-18 ─→ P4-10
...
PD-02 ← P1-01, P1-05, P2-11a  (精简版, 约16h)
```
...
**关键依赖链：**
- **最长串行链：PRE-01 → P1-01 → P2-01 → P2-08 → P2-14 → P2-14-B → P3-08 → P3-09a → P3-10 = 8 步硬串行（实际 15+ 节点含隐藏扇入。降级验收标准见 P3-08）**

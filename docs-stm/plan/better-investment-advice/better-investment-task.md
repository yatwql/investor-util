...
- **阻塞**: 否
- **依赖**: PRE-02
- **状态**: ✅ 已决策（2026-07-20）——885005 不可获取，强制降级路径
- **描述**: **PRE-02 结论：885005（Wind 专属代码）不可获取，CSI 替代代码同样不可用。** 降级方案：P3-07 不扩展 `index.py` 增加 885005，竞争语境只使用沪深300+自定义基金池。在 `prompts.py` 中添加降级说明段落"偏股基金指数暂不可用，以下对比仅基于沪深300"。P3-07 只实现降级说明，无需 `index.py` 扩展。

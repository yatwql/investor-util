...
## Phase 3 — 执行信号与竞争语境 + 事实校验器（~206h，✅ 全部完成）

> **P3-01 ~ P3-13 + P3-16（纯算法层）+ P3-17 ✅ 已完成**（2026-07-20）。**P3-15（TUI 画像问卷）❌ 已取消**——因用户画像降至最低优先级。P3-16 LLM 增强版待评估。详见 `docs-stm/managements/plan.md`。
...
- **依赖**: PRE-02、PRE-02-D
- **状态**: ✅ 已完成（2026-07-20）——自定义基金池 TUI 菜单 + `comparison_indices` 配置 + 多指数竞争语境
...
### P3-08: 竞争语境完整版——夏普对比
- **估时**: 8h
- **文件**: `src/python/llm/prompts.py`（竞争语境段落扩展）、`src/python/report/orchestrator.py`（metrics 数据流贯通）
- **状态**: ✅ 已完成（2026-07-20）——量化指标（夏普/卡玛/波动率/回撤）注入竞争语境【指标对比】段落，metrics 从 `_generate_report_full()` 计算后经 `_fetch_llm_and_news()` → `generate_all_llm()` 全链路贯通，含 8 项单元测试覆盖正常/空/部分键/None 值场景
...
- **估时**: 2h
- **文件**: `src/python/llm/prompts_core.py`
- **阻塞**: 否
- **依赖**: P3-08
- **状态**: ✅ 已完成（2026-07-20）——竞争语境段落末尾追加口径说明脚注（费后净收益 vs 价格指数、含现金 vs 不含、持仓变动三要素）；`_SYSTEM_EXPERT_REVIEW` 新增竞争语境约束，LLM 仅做数据陈述不做结论判断
...
- **阻塞**: 否
- **依赖**: P3-08
- **状态**: ✅ 已完成（2026-07-20）——竞争语境脚注追加幸存者偏差提示，说明对比指数成分股定期调整效应
...
- **描述**: 场外基金实际赎回上限（如"单日上限 10 万"）无法自动获取，需用户配置。在 `_DEFAULT_CONFIG` 中新增 `redemption_limits: dict[str, float]`（code → 单日赎回上限金额）。`liquidity.py` 读取该配置，计算场外品种全量赎回所需天数。未配置的品种显示"需手动确认赎回上限"。
...
### P3-15: TUI 完整画像问卷（❌ 已取消）
- **估时**: 40h（5 天）——已释放
- **文件**: ~~`src/python/tui/profile_questionnaire.py`（新增）、`src/python/config/_config_defaults.py`（新增 `user_profile` 段）~~
- **阻塞**: 否
- **依赖**: 无
- **状态**: ❌ **已取消（2026-07-20）** ——因用户画像降至最低优先级，Phase 4 全部就绪前不评估是否启动。原描述保留作为参考。
- **描述**: ~~风险承受能力/投资期限/投资目标/定投金额/税务身份/经验水平 6 字段问卷，结果持久化到 config。支持"跳过"（用中性通用投资建议垫底）。Phase 5（P5-01~03）未实现时，跳过问卷直接使用中性通用建议。~~

### P3-16: LLM 事实锚定校验器
- **估时**: 24h（3 天）
- **文件**: `src/python/llm/fact_checker.py`（新增）、`src/python/llm/generators_orchestrator.py`（集成）、`src/python/registry.py`（注册）
- **阻塞**: 否
- **依赖**: 无
- **状态**: ✅ 纯算法层已完成（2026-07-20）。LLM 增强版待评估。
...
> **D = Discretionary（独立决策）**，不属于主路线图序列。此阶段经评估后决定关闭，后续如需重启再恢复。

### PD-02: CAPM α 计算 ❌ 已关闭（2026-07-20）
- **估时**: 16h（2 天——精简版，仅输出 α 值，无 t-统计量，解释工作交给 LLM 用自然语言描述）
- **文件**: `src/python/analysis/metrics.py`（新增 `capm_alpha` 函数）
- **阻塞**: 是 ← P1-03（Rf 就绪）+ P1-05（组合日收益率就绪）+ P2-11a（Beta 点估计就绪）
- **依赖**: P1-03、P1-05、P2-11a
- **状态**: ❌ 已关闭（2026-07-20）
- **关闭理由**: α 对个人投资组合的统计效力和适用性存疑——持仓频繁变动+样本量小(5-20品种)+含非股票资产；精简版无显著性检验，LLM 易将噪声值包装成可靠结论造成"伪精确"误导；16h 投入对应不确定的用户价值。后续如需重启，需先完成用户验证（抽取真实用户展示含 α 的报告片段，确认理解度和需求度）。


## Phase 5 — 用户画像（已关闭，按需重启）

> 经评估后决定关闭。Phase D（CAPM α）同步关闭。后续如需重启任一功能，需重新评估 ROI。

### P5-01: 从持仓推断画像——风险级别（Layer 4a）❌ 已关闭（2026-07-20）
- **估时**: 16h
...
### P5-02: 从持仓推断画像——持有期估算（Layer 4a）❌ 已关闭（2026-07-20）
- **估时**: 12h
...
### P5-03: 从持仓推断画像——矛盾处理（Layer 4a）❌ 已关闭（2026-07-20）
- **估时**: 16h
- **文件**: `src/python/profile/inferrer.py`
- **阻塞**: 否
- **依赖**: P5-01、P5-02
...
### P5-05: 衰减模型——画像随时间衰减（Layer 4b）❌ 已关闭（2026-07-20）
- **估时**: 20h
...
### P5-06: 自适应阈值——画像联动再平衡（Layer 4b）❌ 已关闭（2026-07-20）
...
### P5-07: LLM 画像注入——提示语个性化❌ 已关闭（2026-07-20）
- **估时**: 8h
- **文件**: `src/python/llm/prompts.py`
- **阻塞**: 否
- **依赖**: P5-01
...
P3-15(已取消) ─── (TUI 问卷，因用户画像降级已取消)
P3-16(无依赖) ✅ ─── (已实现纯算法层, _COMPUTATION_REGISTRY + orchestrator 串行校验)
...
P5-01 → P5-02 → P5-03 → P5-07  (推断: 风险→期限→矛盾→LLM注入)
          P3-15(已取消) → P5-05(已关闭) → P5-06(已关闭)          (问卷已取消→衰减/自适应阈值一并关闭)
P5-08                           (画像测试 — 已关闭)
...
- 完全独立无依赖：MVP-01、MVP-02、MVP-04、P1-11、P4-09(缓存TTL)、P3-16(事实校验器)

...
| 行业分类/概念板块（规划中） | 东方财富 `push2.eastmoney.com`（三级行业分类 + 概念板块归属） | — |
...
| 菜单 | 功能 | 清除范围 |
|------|------|----------|
| `[1] 更新基础类缓存` | 主动刷新基金业绩、持仓明细、业绩基准、新闻及新闻 LLM 关联分析 | `fund_perf_*`、`fund_hold_*`、`fund_benchmarks.json`、`news_*`、`llm_news_corr_*` |
...
| 6 | **财经新闻热点**（菜单 N/B/L） | 3 源新闻与持仓关键词匹配结果，含标题、来源、时间、关联度。关联关键词按来源富化显示（持仓→蓝色、穿透→紫色、行业→灰色），可选 LLM 二次关联分析 |

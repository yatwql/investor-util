...
| `llm_macro` | 86400（24小时） | 全球政经局势 LLM 分析（指数指纹驱动失效） |
| `llm_expert` | 7200（2小时） | 智囊团深度复盘 LLM 分析 |
| `industry` | 604800（7天） | 行业分类/概念板块缓存 |
| `benchmark` | 2592000（30天） | 业绩比较基准对照表 |
| `profit_forecast` | 86400（24小时） | 机构盈利预测（指数指纹驱动失效） |
| `sector_flow` | 900（15分钟） | 行业资金流向排名（指数指纹驱动失效） |
| `dividend` | 2592000（30天） | 股票历史分红（代码列表指纹驱动失效） |

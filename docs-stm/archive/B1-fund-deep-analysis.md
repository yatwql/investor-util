...
### 6.5 决策回顾：预研结论再确认（1 步）

#### Step B4.5（新增）：预研决策回顾

> **背景**：B0 预研（Week 1 初）的决策信息在文档中闲置约 2 周后才到 B5 实施。
> 此步骤强制重读预研报告，确认数据源方案仍然有效，避免过时决策。

| 字段 | 值 |
|:-----|:----|
| **目标** | 重读 B0 预研报告，确认 B5 实施方案（方案 A/备选/降级 B）仍然有效 |
| **文件** | `docs-stm/plan/notes/data-source-pre-study.md` |
| **操作** | (1) 重读 B0 预研报告中的方案结论和风险标注<br>(2) 快速验证：抽样 3 只持仓股票调用预研确认的 API，验证返回值格式未变<br>(3) 根据当前可用性决策：方案 A（外部 API）→ ✅ 继续；备选（天天基金持仓市值）→ 切换；降级 B（代码段）→ 标注"估算风格"<br>(4) 决策结果写入 B5-1 的备注字段 |
| **验证** | 决策记录明确（三种路线之一），B5-1 按决策路线实施 |
| **预期产出** | commit: "docs: B4.5 预研决策回顾 — 选用{方案A/备选/降级B}" |
| **回滚** | 文档记录无代码影响；如回顾后发现选错路线，可在 B5-1 commit 前修正 |

### 6.6 Phase B5：基金风格漂移检测（2 步）
...
+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+
|  B0  |B1-1  |B1-2  |B1-3~4|B2-1  |B2-2  |B2-3  |B2-4  |B3-1  |B3-2  |B3-3  |B3-4  |B4-1  |B4-2  |B4-3  |B4.5  |B5-1  |
|预研  |经理  |经理  |注册表|变更  |Excel |HTML  |测试  |重叠  |渲染  |报告  |测试  |集中度|集中度|测试  |决策  |风格  |
|      |API   |UT    |+联动  |引擎  |13   |13章  |      |引擎  |辅助  |14   |+HTML |计算  |15   |+HTML |回顾  |判定  |
+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+
| B5-2 |  E   |      |      |      |      |      |      |      |      |      |      |      |      |      |      |
|风格  |全量  |      |      |      |      |      |      |      |      |      |      |      |      |      |      |
|报告+ |回归  |      |      |      |      |      |      |      |      |      |      |      |      |      |      |
|测试  |      |      |      |      |      |      |      |      |      |      |      |      |      |      |      |
+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+
   ^B0完成    ^B1完成        ^B2: 页签13上线   ^B3: 页签14上线   ^B4: 页签15上线   ^B4.5回顾   ^B5: 页签16上线
```

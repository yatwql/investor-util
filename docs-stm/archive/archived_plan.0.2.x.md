...
基金深度分析 4 模块：基金经理变更监控/持仓重合度矩阵/持仓集中度监控/基金风格分析。详见 changelog.md 及 `docs-stm/archive/report-section-order-config/report-section-order-config.md`。

## C 迭代 — 报告序号可配置（v0.2.85 ~ v0.2.86）

报告序号可配置，详见 changelog.md 及 `docs-stm/archive/report-section-order-config/report-section-order-config.md`。

...
> **实施状态（阶段④已实现）**：未新开提示词通道与调用次数，而是接入既有 `signal_pre_digest`（默认开，注入专家复盘与持仓体检）：新增「持仓基本面」与「叙事与数字背离」两路确定性信号行；背离项出现时追加一行要求（请显式指出背离点与核实方向），无背离项时不追加（提示词逐字节不变）；两路信号受 DataSinking 数据底座门禁控制，未就绪自动缺席。审计意见类型/事务所变更信号（下方第三条）仍为后续项。

...
> 归档时间：2026-08-06；2026-08-16 二次合并 changelog.md [0.10.9] ~ [0.10.13] 已发布版本变更记录；2026-09-12 三次合并 changelog.md [0.10.14] ~ [0.10.18] 已发布版本变更记录；2026-09-13 四次合并 changelog.md [0.10.19] 已发布版本变更记录（发布当日并入）
> 原始文件：`docs-stm/managements/changelog.md`
> 涵盖版本：v0.10.1 ~ v0.10.19（2026-08-04 ~ 2026-09-13；v0.10.0 无独立 changelog 段）
> 归档内容：v0.10.x 已发布版本变更记录（仍处开发版本 [0.10.20-dev] 保留在原文件 changelog.md）
...
### 版本发布 v0.10.19（2026-09-13）

- **发布流程**：P2 发布门禁通过（`test-runner --mode verify,regression` **4703 通过 0 失败**（verify 4458 通过 + 1 skipped、regression 245 通过）+ code/doc/task-numbering/semantic-index 四检查全绿 + 发布手动验证 `--mode perf,security` 14 通过）；版本号全链一致化至 v0.10.19（`constants.py` / `pyproject.toml` / `README.md` / 10 份管理文档）；发布数据文档刷新（`collect-test-coverage.py` 实时收集 **6,920** 项，`test-coverage.md` 模式对应测试量表经 `--mode bench --update-docs` 回填本机实测执行计数与环境耗时对照（2026-09-13 采集），功能域/unit 子标记/跨类标记/场景分组四表逐项核对一致；`folders.md` 统计表与实测一致：主程序 **271 文件 67,587 行** / HTML 模板 4 文件 3,877 行 / 架构图 3 文件 315 行 / 辅助脚本 21 文件 7,165 行 / 测试代码 **360 文件 105,975 行** / 测试用例 **6,920 个** / 用户文档 11 文件 5,086 行；`datasource.md` / `datasource-reliability.md` 按实际数据源配置核对——本版未改数据源清单与路由归属，两份文档无需改动）。
- **版本标签**：`git tag v0.10.19` 已打并推送，发布可追溯。
- **已解决项归档**：v0.10.19-dev 已解决项（rf-351 体检数据质量维度基准错误、rf-352 三处「距今多乆」改用交易日、rf-353 转正后口径残留）整体迁入 `docs-stm/archive/v0.10.x/archived_review-findings.0.10.x.md` v0.10.19 章节。
- **本版内容概览**：读侧增强类五项开关转正为默认开启（信号预消化 / 模块级质量分级 / 结构化决策头 / 辩论-条件推理 / 数据源凭据就绪）、完整报告快捷入口 `llm.sh` / `llm.ps1`、时间距离一律以交易日计的三处修复（K 线增量跳空判定 / 停更因子剔除 / 新买入观察期防护）与体检数据质量维度基准修正、架构约束新增「时间距离一律以交易日计」、CLI 手册补 TUI 菜单→CLI 命令对照表、已实现设计文档与已完成内容并入 v0.10.x 归档。

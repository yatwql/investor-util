...
> 归档时间：2026-08-05（设计文档 + 完成项摘要）；2026-08-05 二次合并 plan.md 已完成事项记录；2026-08-07 追加 plan-25 Web 持仓输入模式 / plan-26 Web 配置编辑 / README SVG 架构图；2026-08-08 追加 env-benchmark-doc-update（--update-docs 环境耗时对照自动更新）；2026-08-16 三次合并 plan.md 已完成事项记录（plan-8/25/26/27/28 P4 实验功能项）；2026-09-10 四次合并 plan.md 已完成事项记录（plan-29 DeepSeek 峰谷定价周末闲时）；2026-09-11 五次归档——`docs-stm/plan/` 全部 14 份外部借鉴系列设计文档（plan-30~plan-38 + 指纹提示词覆盖）按借鉴来源分四目录迁入；2026-09-12 六次合并 plan.md 已完成事项记录（plan-29 引用、plan-40 / plan-41 完成态与 P4 实验功能表 plan-30~plan-41 整体迁入）；2026-09-15 七次归档——`docs-stm/plan/` 两份已实现设计文档（plan-42 DataSinking 财报摘要、plan-43 基本面数据源与财务指标）迁入 `datasink-financial-report-digest/` 与 `financial-indicator-source/`，plan.md 完成项（plan-42 / plan-43）整体迁入
> 原始文件：`docs-stm/managements/plan.md`（当前迭代部分）
> 涵盖版本：v0.10.0 ~ v0.10.19（2026-08-03 ~ 2026-09-13）；plan-42 于 v0.10.19-dev 实现（2026-09-13 ~ 09-14）、plan-43 于 v0.10.20-dev 实现（2026-09-15）；plan-8 于 v0.10.10 实现、plan-25/26/27/28 于 v0.10.12 实现、README SVG 于 v0.10.13 实现（2026-08-06/07）、plan-29 于 v0.10.15 实现（2026-08-28）、plan-30~plan-38 外部借鉴系列于 v0.10.16~v0.10.17 实现（2026-09-10）、plan-39~plan-41 于 v0.10.19-dev 实现（2026-09-11 ~ 2026-09-12）
> 归档内容：本迭代已实现的计划项（plan-8 + plan-17~plan-43）设计文档 + 完成项摘要 + 推荐实施顺序 + 发布门禁记录（P0/P1/P2/P3 已完成事项记录 + P4 已随发布版本实现项自 plan.md 整体迁入）。plan-30~plan-41 的完成项摘要已随六次合并并入本文件；plan-42 / plan-43 的完成项摘要随七次合并并入本文件。
...
### P1 — 已完成（plan-42 / plan-43 完成态，2026-09-15 七次合并迁入）

#### ✅ `plan-43` 持仓个股基本面数据源主备与财务指标提取 — 已完成（2026-09-15）
...
**实现（2026-09-15，5 个提交）**：① 结构化指标主源（akshare 宽表归一 + `financial_indicator` 域与契约）；② 全文解析备用支路（`datasink_indicator`，实测把「读表格」改为「锚点 + 限窗取值」并收窄到「公司简介和主要财务指标」章节）；③a 财务指标报告章（指标列 + 质量档 + 年度趋势 + 当前 PE/PB）；③b 真实历史估值分位（TTM 口径，法定披露截止日生效以避免前视偏差）+ DataSinking 数据底座门禁（未就绪静默回原样）；④ LLM 侧注入（基本面信号 + 叙事-数字背离检测，接入既有 `signal_pre_digest`，不新增调用）。阶段⑤（文档与门禁收尾）随发布执行。

设计文档：[`financial-indicator-source-design.md`](financial-indicator-source/financial-indicator-source-design.md)（含数据源事实、主备结构、标准字段契约、全文解析设计、派生分析、配置与开关、配额与降级、测试计划、实施顺序与风险）。分三阶段实施，详见其 §13。

#### ✅ `plan-42` 持仓个股财报摘要（DataSinking 全文本财报接入）— 已完成（2026-09-14）
...
设计文档：[`datasink-financial-report-digest-design.md`](datasink-financial-report-digest/datasink-financial-report-digest-design.md)（含语义命名索引、文件级落点、限速与配额设计、配置 Schema、测试计划与风险）。
...
- **七次合并（2026-09-15）**：`docs-stm/plan/` 两份已实现设计文档迁入本目录——plan-42 DataSinking 财报接入设计 → `datasink-financial-report-digest/`，plan-43 基本面数据源主备与财务指标提取设计 → `financial-indicator-source/`（文档头部状态行同步改为已实现态、文档间交叉引用与 plan.md / changelog 引用改指归档路径）；`docs-stm/managements/plan.md` 的 P1 区两条完成项（plan-42 / plan-43）整体迁入本文件新增小节，plan.md 的 P1 / P4 两区自此均为「无待办项」，`docs-stm/plan/` 再次成为空目录。
- 版本号：本归档涵盖已发布版本 v0.10.0 ~ v0.10.4（当前开发版本 v0.10.5-dev，归档时点为 2026-08-05），归档目录按版本段命名 v0.10.x。三次合并后归档范围扩展为已发布 v0.10.0 ~ v0.10.13（2026-08-03 ~ 2026-08-14）；五次归档后扩展为已发布 v0.10.0 ~ v0.10.17（2026-08-03 ~ 2026-09-10）；六次合并后扩展为已发布 v0.10.0 ~ v0.10.18（2026-08-03 ~ 2026-09-12）；七次合并后扩展为已发布 v0.10.0 ~ v0.10.19（2026-08-03 ~ 2026-09-13）。

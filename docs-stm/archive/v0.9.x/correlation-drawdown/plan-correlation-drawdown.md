...
**对 `rf-1` 的依赖（已解除）**：原设计顾虑相关性矩阵涉及 15+ 品种历史数据获取，全量串行时每品种 ~1-2s HTTP、合计 15-30s 额外等待。**rf-1 批量并行已随 v0.8.x 落地并全链路生效**——`BatchDispatcher` 已应用于行情获取（`market_value.py`，TPE 8 并发）、基金排名（`fund.py`）、行业分类（`industry.py`）等批量链路。全品种历史数据获取可直接复用并行链路，串行瓶颈已消除。因此无需"先实现计算与渲染、待 rf-1 后再追加批量优化"的两阶段策略，plan-2 可按并行路径独立实施。
...
| 数据获取编排 | `orchestrator.py` 获取全品种历史数据（复用 `BatchDispatcher` 并行链路，rf-1 已落地）→ 调用纯计算函数 → 结果写入 `pipeline_data['correlation_data']`（C19 新键） | 0.5 |

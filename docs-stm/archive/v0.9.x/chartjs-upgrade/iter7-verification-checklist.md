...
> 状态：⏳ 部分完成（rf-113，review-findings.md P1）——① 全过、② 2.1~2.3 过、③ 3.2~3.4 过；待补验：② 2.4、③ 3.1、④、⑤、⑥
...
> **验证（2026-08-06 Windows，另机）**：ok/degraded 场景 6/6 图渲染 + 全部图 tooltip 可用；empty 场景 4/6 图渲染 + tooltip（资产构成/雷达空数据占位，预期）；offline 场景引擎缺失守卫生效。✅ ① 通过（Chrome + Firefox 实测，Edge 未测——同 Chromium 内核，S2 升级时顺带补验）。期间修复：rf-248 动态脚本顺序、rf-249 折线/雷达 tooltip 触发、rf-251 空数组显式守卫。
...
> **操作步骤（用真实报告，不用 test-chart.html——微信验证的是用户实际收到的报告）**：
> - **前置**：菜单 L/B 生成一份完整报告，拿到 `reports/` 下 `.html`
> - **4.1 链接访问**：报告目录起本地静态服务（如 `.venv/bin/python -m http.server 8000`）或部署到 http/https → 微信内点链接打开 → 6 图渲染正常
> - **4.2 file:// 访问**：报告 `.html` 通过**文件传输助手**发到手机 → 微信点开 → 图表渲染
> - **4.3 file:// 兜底**：若 X5 沙箱拦 file:// 导致图表不渲染 → 验证**无 JS 报错 + 明细表格兜底可读** → 也算过
> - **4.4**：竖屏/横屏切换，图表自适应不溢出（A4）
...
> **操作步骤（用真实报告，fallback 文本在真实报告里才有意义）**：
> 1. 生成一份完整报告，打开报告 `.html`
> 2. `F12` → **Settings**（齿轮）→ **Preferences** → 勾选 **Disable canvas**（模拟无 canvas 环境）
> 3. 刷新页面 → 6 图区域应显示内嵌 fallback 文本（如「图表无法显示，数据见报告明细表格」）
> 4. 检查 6.2：浅色模式下图表文本可读（对比度 ≥ 4.5:1 目测）
> 5. 测完**取消勾选** Disable canvas → ✅
...
| 项 | 标题 | 结果 | 备注 |
|:--:|:-----|:----:|:-----|
| ① | 6 图渲染 + 交互 | ☐ 通过 / ☐ 不通过 | ✅ 已过（Windows Chrome+Firefox，2026-08-06；Edge 未测） |
| ② | 打印降级 | ☐ 通过 / ☐ 不通过 | ✅ 2.1~2.3 已过（2x DPI/浅色/不跨页）；2.4 afterprint 待补验 |

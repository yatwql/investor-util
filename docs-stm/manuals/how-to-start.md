...
轻量 Web 界面：浏览器内上传持仓 Excel → 选择报告格式 → 实时查看生成进度 → 预览 HTML / 下载 Excel，无需终端交互。
...
启动后浏览器访问 **http://127.0.0.1:8000**。默认仅监听本机回环地址（更安全）；如需局域网内其他设备访问：

```bash
./scripts/launch.sh web --host 0.0.0.0
```

> **⚠️ 局域网访问提示**：`--host 0.0.0.0` 后局域网内任何设备均可访问，Web 模式**无内建认证**，请仅限可信网络使用；公网部署必须前置反向代理做 basic auth。

**常用参数**：

| 参数 | 说明 |
|:-----|:-----|
| `--host ADDR` | 监听地址，默认 `127.0.0.1`（仅本机）；局域网访问显式用 `0.0.0.0` |
| `--port PORT` | 监听端口，默认 `8000`（端口被占用时启动会报错并提示换端口） |
| `--config PATH` | 备用配置文件路径，默认 `data/config/config.json` |

> **Web 模式使用要点**：
> - 首页「③ 配置编辑」面板与 TUI 主菜单可编辑项**完全一致**（7 组：路径/报告章节/子模块/匿名化/对比指数池/LLM 开关/实验性功能），**即改即存**写入共享配置文件（config.json / llm_settings.json / features.json）并写前自动备份 `.bak`——详见 [Web 浏览器模式使用指南](how-to-use-web-mode.md) §4。
...
CLI 模式的完整命令参考（全局参数 / `report` / `cache` / `whatif` / `check-sources` / `view-logs` / `doctor` 子命令、使用示例、常用命令速查、退出码、最佳实践）见 [CLI 命令行模式使用指南](how-to-use-cli-mode.md)。
...
定时任务配置详见[CLI 命令行模式使用指南](how-to-use-cli-mode.md) §13「定时任务」。

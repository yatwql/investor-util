...
`official_source` 启用时，API 返回 `f100` 字段表示实时交易状态，结果缓存策略按当前时间判断：

| 时段 | `f100` 典型值 | 含义 | API 缓存 TTL |
|:----|:------------:|:-----|:------------:|
| 09:30 前 | `0` | 未开盘（盘前） | 7 天（盘后长效缓存） |
| 09:30–11:30 | `1` | 交易中（上午盘） | 60 秒（盘中高频刷新） |
| 11:30–13:00 | `3` | 午间休市 | 60 秒（午休后快速恢复） |
| 13:00–15:00 | `1` | 交易中（下午盘） | 60 秒（盘中高频刷新） |
| 15:00 后 | `2` | 已收盘（盘后） | 7 天（盘后长效缓存） |
...
| `history_fund_otc` | `history_fund_otc_{code}*.json` | 30 天 | — | 场外基金历史净值（天天基金），F2 组合走势计算输入 |
...
`degradation``degradation` 控制数据降级行为，定义各层级数据在连续失败或空数据返回时切换到降级模式的阈值。
...
缓存分组由 `src/python/registry.py` 中的模块注册表驱动。每个模块注册时指定所属分组（`cache_groups` 字段），`cache:clear_by_group()` 遍历注册表，只清除匹配分组的模块缓存文件。新增缓存模块时只需在注册表中声明分组，无需修改菜单代码。

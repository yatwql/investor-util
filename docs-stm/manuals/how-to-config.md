...
> **报告章节与增强子模块（数据质量仪表盘/市场温度/行业Beta子表/候选基金比较/成本流水/估值分位/持仓个股财报摘要/财务指标）不在 `[P]`**——它们已并入功能开关注册表的「报告章节与增强」组，统一在菜单 `[S]` 的报告块配置（见 [M. 功能开关](#m-功能开关featuresjson)）。
...
`data/config/features.json` 提供 **28 项功能开关**的运行时覆写（含「报告章节与增强」8 项，统一登记于功能开关注册表）。文件仅需列出需覆写的开关，未列出的保持代码内置默认值：

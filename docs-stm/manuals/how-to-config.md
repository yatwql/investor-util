...
| `datasink.*` | `{plan: "free", ...}` | DataSinking 全文本财报配置：`plan`（free/yearly，决定限速与日配额默认值）、`requests_per_second` / `daily_quota`（0=按 plan 自动）、`sections`（取用章节）、`max_chars`（摘要截断）、`doc_types`（文种优先级）。免费档 3 请求/秒、8191 篇/日；仅覆盖 A 股 | 手动编辑 |
...
| `datasource_credential_ready` | true | 数据源凭据就绪指引（接入需 key 的源时补一行 `CredentialSpec` 声明即可：链路**主动跳过**缺凭据的源并给出「缺什么/去哪申请」可读指引、`check-sources` 出 `⏭️` 跳过态与就绪摘要、`doctor` 出「数据源凭据」组，均复用既有面、不新增界面。**当前全部数据源免费，声明表为空**，开启后可见的就是「均无需凭据（免费源）」。凭据只从环境变量读，**值永不落日志与报告**）。**默认开启**——声明表为空时对报告产物零影响，且只读诊断不耗 LLM、不写盘 |

...
> **数据获取链路**：股票/ETF 走腾讯 K 线历史 → 新浪备用；OTC 基金走天天基金历史净值 → 东方财富备用（v0.4.4+ 空结果递补）。东方财富净值 API 使用 `pageSize=20` 分页获取（最多约 200 条≈10 个月），增量缓存逐步积累更久数据。
>
> **累计收益率起算**（v0.4.4+）：从 ≥80% 持仓覆盖的日期起算，避免因 QDII/债券基金数据起点较晚导致早期组合市值偏低、收益率虚高。早期数据保留在走势图上但排除出收益率计算。起算日期在日志中以 `[history] 收益率起算点调整` 记录。

...
全部 19 项开关分两组，**两组在 TUI 菜单 [S] / Web 配置面板 / CLI 中同样可切换**。
...
**常规组（10 项，默认开；关闭即收回对应能力。其中量化指标与交互图表**关闭会改变报告产物内容**——Web 配置面板对这些项标注「（影响报告）」；系统自检与数据源适配只影响内部路径与入口显隐，不标注）**：
...
> 以上 19 项为**全部**功能开关清单（默认值与代码 `features.py::feature_switch_registry` 一致）。features.json 仅需列出需覆写的开关，未列出的保持默认值。

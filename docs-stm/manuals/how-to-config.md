...
| 实验性功能 | `llm_debate_procon` / `llm_debate_conditional` / `llm_debate_qa_concentration` / `decision_reflection` / `signal_pre_digest` / `module_quality_gate` / `decision_header_parse` / `signal_ledger` / `datasource_adapter` / `datasource_credential_ready` | `[S]` 实验块 |

...
- **基金深度分析**（基金经理变更监控/持仓重合度矩阵/持仓集中度监控/基金风格分析）—— 对应 `enable_b_series`
...
菜单 E/B/L 生成范围不同：E 为基础页签（1~5，仅始终显示的 5 个核心模块，不含 B 系列基金分析、不含新闻、不含历史走势），B 含 B 系列+新闻模块+历史走势（不含 LLM 分析模块），L 为全量（1~17）。各菜单对应的页签范围详见[菜单操作手册](how-to-menu.md#报告内容对照)。
...
A: 通常不需要。缓存文件与程序版本解耦，TTL 过期后自动失效。仅当版本更新说明中特别标注"缓存结构变更"时才需手动清理（会写在 `docs-stm/managements/changelog.md` 中）。升级后若遇到异常数据，可先执行菜单 `[3]` 清理过期缓存。

...
A: 暂不支持多文件同时对比分析。但可通过执行多次生成（每次选择不同 xlsx 文件），分别生成多份报告后进行手动对比。
...
A: 推荐方法：先后生成两份报告（如上周和本周），对比 Excel 报告中"市值核算明细表"页签的盈亏和市值列；或对比两份 HTML 报告中的指数行情和账户汇总区域。程序不内置 diff 功能。

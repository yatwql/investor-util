...
菜单 E/B/L 生成范围不同：E 为基础页签（始终显示的 6 个核心模块，另含组合演进页签——开启 `enable_portfolio_evolution` 时显示；不含基金深度分析、不含新闻、不含历史走势），B 在 E 基础上加基金深度分析+新闻+历史走势（不含 LLM 分析模块），L 为全量（1~21）。各菜单对应的页签范围详见[菜单操作手册](how-to-menu.md#报告内容对照)。

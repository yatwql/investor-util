...
A: 如果不同持仓的历史数据起止日期差异较大（如 QDII 基金最早数据仅到 2025-09、债券基金仅到 2026-03），早期组合因基金数量不足导致市值偏低，收益率可能异常虚高。累计收益率从 ≥80% 持仓覆盖的日期起算，早期数据仍保留在走势图上但排除出收益计算。如需验证，可查看日志中的 `[history] 收益率起算点调整` 记录。

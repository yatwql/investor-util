...
**Q: 我注意到报告中多了"流动性风险"的分析，是怎么计算的？**

A: 流动性分析分为两类：
- **场内品种**（股票/ETF）：系统自动获取近 20 日日均成交额，按"持仓市值 ÷ 日均成交额"计算变现天数。如果少于 1 天标注"当日可卖出"，超过标注"需多日卖出"。
- **场外基金**（OTC）：需要您在 `config.json` 的 `redemption_limits` 中配置各基金的单日赎回上限（如 `"000001": 100000` 表示单日上限 10 万）。配置后系统自动计算全量赎回所需天数；未配置的品种显示"需手动确认赎回上限"。

K 线数据不可用时默认假设流动性充足，不产生告警。

**Q: 如何配置场外基金的赎回上限？**

A: 编辑 `data/config/config.json`，在 `redemption_limits` 字段中按 `{基金代码: 单日赎回金额上限}` 的格式添加配置，例如：
```json
"redemption_limits": {
  "000001": 50000,
  "110011": 100000
...
```
配置后程序可自动计算该基金全部赎回所需天数。菜单 `R` 刷新配置立即生效，无需重启。

**Q: 报告中的"再平衡信号"是什么意思？**

A: 再平衡信号是系统根据您的持仓市值占比和配置目标自动检查的调整建议，包含两类：
- **单品种超限**：某品种市值占比超过阈值（默认 15%，可配置），建议部分止盈或减仓
- **权益/固收偏离**：权益类总仓位偏离目标配置超过 5% 时触发大类调整建议

每个信号附带置信度（high/medium/low），您可以根据置信度决定是否采纳建议。
...
A: 可通过以下方式调节：① 增大 `rebalance.threshold` 提高超限容忍度；② 增大 `rebalance.silence_days` 延长静默期（默认 30 天，同一品种触发后 N 天内不重复告警）；③ 切换 `profile` 到 `aggressive` 使用更宽松的阈值预设。静默期状态持久化到 `data/cache/rebalance_silence.json`，跨会话生效。

**Q: 报告中的竞争语境对比用了哪些指数？**

A: 默认使用沪深300 + 中证500 + 中证全债三个指数做多维度对比，展示组合与各基准的收益（年初至今/近1年）和风险指标（夏普/波动率/最大回撤）。可通过 `config.json` 的 `comparison_indices` 字段自定义指数池，支持多指数并行对比。报告中附有口径说明脚注和幸存者偏差提示。
...
A: 可以。完成一次菜单 L 后，可再次执行菜单 E/B 生成指定格式报告。程序会复用已获取的缓存数据（行情、新闻、LLM 分析等），无需重新请求外部 API。建议顺序：先执行一次 L（全量，含 LLM），如需单独再出不含 LLM 的版本，随后执行 B 或 E 即可。
...
A: 拉取最新代码后版本号记录在 `src/python/constants.py` 的 `APP_VERSION` 中，更新日志见 `docs-stm/managements/changelog.md`。如果版本号未变，说明当前代码相较您本地未新增版本号变更。
...
A: 暂不支持单独页签输出。菜单 E/B/L 均生成完整的报告文件（全部页签），不可跳过中间页签。但菜单 E/B 不含 LLM 增补内容（菜单 E 仅生成 Excel，菜单 B 生成 Excel+HTML），可据此选择最简输出。
...
A: 系统根据持仓品种的代码和名称自动判断交易币种（A 股/国内基金→CNY，港股通→HKD，QDII/海外基金→USD），按市值加权汇总各币种占比。在智囊团深度复盘（菜单 L）中展示人民币/港币/美元比例，非人民币资产占比>0%时会附加汇率波动风险提示。港股通品种会额外说明"港币计价，实际换汇成本隐含在汇率中"。

**Q: 报告末尾的 LLM 用量信息是什么意思？**

A: 每次通过菜单 L 生成报告后，报告末尾会显示 LLM Token 用量摘要：各模块的输入/输出 Token、耗时、估算费用。系统默认设定了每份报告 8K 输入 Token 的预算上限，超出时程序会告警但不会截断报告内容。这些数据有助于了解不同 LLM 模块的 Token 消耗分布，为后续选择更经济的模型提供基线参考。

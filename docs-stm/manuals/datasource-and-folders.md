...
│       │   │   ├── classification_utils.py # 资产类型判定工具 — 股票/ETF/债券/QDII 统一判定入口
...
│       │   │   ├── test_chain_edge.py    # Provider Chain 异常场景 — 全链 fallback/超时/空响应（4 项）
...
│       │   │   ├── test_fund_manager.py  # 基金经理抓取 — _extract_fund_manager / F2 流程（14 项 B2）
...
│       │   │   ├── test_classification_utils.py # 资产类型判定工具 — 股票/ETF/债券/QDII 统一判定（42 项）
...
│       │   │   ├── test_registry.py      # 中央注册表 — 模块注册/TTL 映射/设置键派生/section_order 测试（21 项）
│       │   │   ├── test_registry_edge.py # 注册表异常场景 — 查找失败/section_order 异常输入（8 项）
...
│           ├── __init__.py               # 子包标记（空文件）
│           ├── basic/                    # 基础业务场景 S0a-S0d + S1-S5 + S21-S33 + C-P1b + S-P1~S-P10（127 项）
│           │   ├── __init__.py           # 子包标记（空文件）
...
│           │   └── test_scenario_penetration.py # S-P1~S-P10：穿透 TOP10 场景 — 股票/债券/ETF/主动/QDII/黄金ETF/联接/交叉持股（37 项）
...
│       │   ├── unit/report.html           # 单元测试报告（标记 -m "unit"，2219 项）
│       │   ├── standard/report.html       # 常规单元报告（标记 -m "unit and not (edge or data)"，1938 项）
│       │   ├── scenario/report.html       # 场景测试报告（标记 -m "scenario"，277 项）
│       │   ├── regression/report.html     # 回归测试报告（标记 -m "scenario"，模式别名，277 项）
│       │   ├── verify/report.html         # 合入验证报告（标记 -m "scenario or unit_core or unit_providers or unit_fetcher"，944 项）
...
│       │   ├── all/report.html            # 全量测试报告（无标记筛选，2521 项）

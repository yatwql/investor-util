...
- **已接入域**：**行情**（`price_stock` 第三槽，官方快照）、**历史日 K**（`history_stock` 第三槽，前复权，支持增量）、**交易日历**（akshare 失败时的官方兜底）、**财务指标**（`financial_indicator` 链路**第三槽**：主源不可用时由三张官方合并报表派生多期指标，口径与主源对齐）、**基金披露持仓**（`fund_hold` 链路**备源**：天天基金全链不可用时接管，载荷经同一归一器落到同一契约；联接基金直接返回目标 ETF，省去 HTML 探测）。provider 层剩余域（**情绪面**：涨跌停池/连板天梯/龙虎榜；以及复权因子事件流的消费方「分红流水漏记校验」）按 `plan.md` 计划表推进
### 财报全文（DataSinking）

...
| 输出端 | 展示形式 | 说明 |
|:-------|:---------|:-----|
| **Excel 报告** | 独立页签「LLM API 用量」 | 顶部汇总区 + 下方模块明细表，状态列带条件颜色填充 |
| **HTML 报告** | 报告「LLM API 用量」章节（底部） | 与 Excel 格式一致 |

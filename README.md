...
- **双持仓 diff 对比** — 菜单 `W` / CLI `whatif` 选择基准与目标两份持仓 xlsx，生成独立 `调仓模拟.xlsx`（调仓摘要 / 分类配置对比 / 持仓变动明细 3 页签）与 `调仓模拟.html`，与主报告 21 页签互相独立
...
- **数据源健康检查** — 每次报告生成时后台并行执行全量数据源 HTTP 连通性检测，结果存入 `data/state/datasource_health.jsonl` 并实时反映在报告 #20 数据源可用性矩阵章节

...
- **Excel 报告** — 12 个功能页签：投资分析汇总、市值核算明细表、持仓分类表、资产穿透TOP10、基金业绩分析、财经新闻热点与持仓关联分析、智能预警、全球政经局势（LLM）、智囊团深度复盘（LLM）、持仓体检报告（LLM）、穿透深度分析（LLM）、LLM API 用量统计
...
- **智能预警** — 行业资金流向联动预警 + 新闻情绪聚合，基于已有数据自动计算，不依赖 LLM（菜单 N/B/L 自动附带）
...
| 菜单 | 核心报告 | 财经新闻 | 智能预警 | 全球政经局势（LLM） | 智囊团深度复盘（LLM） | 持仓体检报告（LLM） | 穿透深度分析（LLM） |
|------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **E** | ✅ | — | — | — | — | — | — |
| **N** | ✅ | ✅ | ✅ | — | — | — | — |
| **H** | ✅ | ✅ | ✅ | — | — | — | — |
...
| 7 | **11.智能预警**（菜单 N/B/L） | 行业资金流向联动预警 + 新闻情绪聚合，自动附带，不依赖 LLM |
| 8 | **7.全球政经局势**（菜单 L） | LLM 基于指数行情和持仓结构生成的宏观分析 |
| 9 | **8.智囊团深度复盘**（菜单 L） | LLM 三阶段圆桌会议：召集令→辩论→定音锤，含调仓建议和风险预警 |
| 10 | **9.持仓体检报告**（菜单 L） | LLM 从风险分散度、流动性、收益合理性、成本结构四维度量化评分并给出改进建议 |
| 11 | **10.穿透深度分析**（菜单 L） | LLM 从行业集中度、国别/币种暴露角度生成深度分析，含行业集中度仪表盘和外汇风险敞口 |
| 12 | **12.LLM API 用量**（菜单 L） | LLM 各模块 Token 消耗、费用估算及 per-module 模型/用量明细 |

### HTML 报告

单页渲染以上全部 12 个功能页签（共 12 个章节，含智能预警 + LLM API 用量统计），响应式 CSS 自适应桌面/移动端。额外特性：
- 盈亏正数红色、负数绿色着色

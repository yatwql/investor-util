...
| **专业的 Excel 报告** | 最多 **19 个条件页签**分七组：投资分析汇总、持仓明细与分类、资产穿透 TOP10、基金业绩 5 级评级、持仓结构与集中度、风格与因子分析、财经新闻热点关联、组合历史走势与回撤、行动建议，以及可选的持仓个股财报摘要、财务指标，与 LLM 全球政经局势 / 智囊团深度复盘 / 持仓体检 / 穿透深度分析 |
...
- **Excel 报告** — 最多 21 个条件页签，分七组：
  ...
  - **财报摘要**（1 个，可选）：持仓个股财报摘要——开启 功能开关 `financial_report_digest` 且配置 DataSinking key 时生成（否则不出现）
  - **组合演进**（1 个）：组合演进——启用 `enable_portfolio_evolution` 时生成（数据不可用时占位）
  - **基金深度分析**（3 个）：基金经理变更监控、持仓结构与集中度（一章三区块：重合度 + 相关性 + 集中度）、风格与因子分析（一章三区块：风格表 + 因子回归 + 行业 Beta 子表）——有基金持仓时自动显示

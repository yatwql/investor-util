...
- **Excel 报告** — 最多 20 个条件页签，分七组：
  ...
  - **基金深度分析**（5 个）：基金经理变更监控、持仓关系矩阵（一章两区块：重合度 + 相关性）、持仓集中度监控、基金风格分析、因子暴露分析——有基金持仓时自动显示
...
- **币种敞口分布** — §14 智囊团深度复盘（币种敞口段落）与 §16 穿透深度分析（国别/币种暴露段落）展示 A股/港股/美股 按市值加权占比，非人民币资产>0% 时附加汇率波动风险提示
...
- **数据源健康检查** — 每次报告生成时后台并行执行全量数据源 HTTP 连通性检测，结果存入 `data/state/datasource_health.jsonl` 并实时反映在报告 #19 数据源可用性矩阵章节

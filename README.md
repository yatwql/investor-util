...
| **专业的 Excel 报告** | 最多 **17 个条件页签**分七组：投资分析汇总、持仓明细与分类、资产穿透 TOP10、基金业绩 5 级评级（含基金经理变更监控块）、持仓结构与集中度、风格与因子分析、财经新闻热点关联、组合历史走势与回撤、行动建议，以及可选的持仓基本面（财务指标 + 财报摘要），与 LLM 全球政经局势 / 智囊团深度复盘 / 持仓体检 / 穿透深度分析 |
...
`--feature NAME=VALUE` 是它的补集：面向**全部 20 项开关**、**双向**（`=off` 亦可），仅本次运行、不写入 `features.json`，在其之后应用，故 `--experiment all --feature signal_ledger=off` 表示「其余实验功能全开、只关信号沉淀」。两个参数同样适用于 `doctor` / `check-sources` / `view-logs` / `cassettes` 等早返回命令。
...
- **Excel 报告** — 最多 21 个条件页签，分七组：
  - **基础核算**（5 个）：投资分析汇总、持仓明细与分类、资产穿透TOP10、基金业绩分析、数据源可用性矩阵——每次生成必有
  - **持仓基本面**（1 个，可选）：持仓基本面（财务指标区块 + 财报摘要区块）——`financial_indicator` / `financial_report_digest` 两开关各控一块，任一块开启且数据可用时出现（财报摘要需配置 DataSinking key）
...
- **分红历史分析** — 个股历年分红自动汇总，显示年均股息率（持仓明细与分类·分类汇总区块）和年均股息金额（穿透TOP10）
...
- **再平衡监控** — 单品种市值超限告警 + 权益/固收大类偏离检测，置信度分级（high/medium/low）。阈值三档预设（保守10%/3%/稳健15%/5%/进取25%/8%）或自定义，`silence_days` 可设静默期避免重复告警
...
- **币种敞口分布** — §13 智囊团深度复盘（币种敞口段落）与 §15 穿透深度分析（国别/币种暴露段落）展示 A股/港股/美股 按市值加权占比，非人民币资产>0% 时附加汇率波动风险提示

### 🔄 调仓 What-if 模拟（独立报告）

- **双持仓 diff 对比** — 菜单 `W` / CLI `whatif` 选择基准与目标两份持仓 xlsx，生成独立 `调仓模拟.xlsx`（调仓摘要 / 分类配置对比 / 持仓变动明细 3 页签）与 `调仓模拟.html`，与主报告 20 页签互相独立
- **指定生效日时序回测** — 可选 `--effective-date YYYY-MM-DD`（opt-in 联网获取生效日之后历史行情），按 as-if 市值（份额 × 每日价格）计算基准/目标归一化净值曲线，对比区间收益、年化收益、年化波动率、夏普比率、最大回撤 5 项指标；回测为假设推演不构成收益承诺，数据不足时自动降级
...
- **基金经理变更监控**（基金业绩分析章内区块）— 快照式变更检测，1/3/6 月多窗口对比，变更信息预警着色，支持 ETF 和场外基金
- **持仓重合度矩阵** — Jaccard + Overlap Ratio 双指标，基金两两配对，识别伪分散风险
- **集中度监控+风格漂移** — TOP3/5/10 三级预警 + 市值/PE 加权风格六宫格 + 曼哈顿距离漂移评分
- **风格因子回归 + 行业 Beta** — 风格与因子分析章内价值/成长/质量 3 因子时间序列 OLS 回归，输出 β 系数、显著性、风格归属占比，含基准对照与行业 Beta 子表

...
- **Python ≥ 3.11**（3.10 已于 2026‑10 终止支持，不再兼容）
...
- **智囊团深度复盘** — 三阶段圆桌会议（召集令→辩论→定音锤）；可选辩论模式（菜单 [S] 开启）：M1 三段式正反辩论(pro 绿/con 红/synthesis 金)、M2 条件推理(情景化分析)、M3 集中度问答。末尾自动附上：
...
- **持仓体检报告** — 从风险分散度、流动性、收益合理性、成本结构四维度量化评分（满分100），附改进建议

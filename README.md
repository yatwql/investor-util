...
| **专业的 Excel 报告** | 最多 **21 个条件页签**分七组：投资分析汇总、市值核算、资产穿透 TOP10、基金业绩 5 级评级、持仓关系矩阵、风格与因子分析、财经新闻热点关联、组合历史走势与回撤、行动建议，以及可选的持仓个股财报摘要、财务指标，与 LLM 全球政经局势 / 智囊团深度复盘 / 持仓体检 / 穿透深度分析 |
...
- **Excel 报告** — 最多 21 个条件页签，分七组：
  ...
  - **基金深度分析**（4 个）：基金经理变更监控、持仓关系矩阵（一章两区块：重合度 + 相关性）、持仓集中度监控、风格与因子分析（一章三区块：风格表 + 因子回归 + 行业 Beta 子表）——有基金持仓时自动显示

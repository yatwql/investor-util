...
| **专业的 Excel 报告** | 最多 **20 个条件页签**分七组：投资分析汇总、市值核算、资产穿透 TOP10、基金业绩 5 级评级、持仓关系矩阵、风格与因子分析、财经新闻热点关联、组合历史走势与回撤、行动建议，以及可选的持仓个股财报摘要，与 LLM 全球政经局势 / 智囊团深度复盘 / 持仓体检 / 穿透深度分析 |
...
- **Excel 报告** — 最多 20 个条件页签，分七组：
  - **基础核算**（6 个）：投资分析汇总、市值核算明细表、持仓分类表、资产穿透TOP10、基金业绩分析、数据源可用性矩阵——每次生成必有
  - **财报摘要**（1 个，可选）：持仓个股财报摘要——开启 `report_submodules.financial_report_digest` 且配置 DataSinking key 时生成（否则不出现）

...
| **专业的 Excel 报告** | 最多 **19 个条件页签**分七组：投资分析汇总、市值核算、资产穿透 TOP10、基金业绩 5 级评级、持仓关系矩阵、风格与因子分析、财经新闻热点关联、组合历史走势与回撤、行动建议，以及 LLM 全球政经局势 / 智囊团深度复盘 / 持仓体检 / 穿透深度分析 |
...
`--feature NAME=VALUE` 是它的补集：面向**全部 20 项开关**、**双向**（`=off` 亦可），仅本次运行、不写入 `features.json`，在其之后应用，故 `--experiment all --feature module_quality_gate=off` 表示「其余实验功能全开、只关质量分级」。两个参数同样适用于 `doctor` / `check-sources` / `view-logs` / `cassettes` 等早返回命令。
...
- **双持仓 diff 对比** — 菜单 `W` / CLI `whatif` 选择基准与目标两份持仓 xlsx，生成独立 `调仓模拟.xlsx`（调仓摘要 / 分类配置对比 / 持仓变动明细 3 页签）与 `调仓模拟.html`，与主报告 19 页签互相独立

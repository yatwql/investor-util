...
- **LLM 智能分析** — 支持 Claude / OpenAI / DeepSeek API，结果按策略缓存（2h~24h），持仓变更时通过缓存指纹自动失效
...
- **Beta 置信区间 + 统计检验** — 95% 置信区间、t-统计量、p 值，数据不足时标注"可靠性有限"
- **情景分析** — ±1σ/±2σ 共 6 种涨跌情景下组合预期回撤/收益，含市场/行业/汇率三张情景表；置信区间传播（Beta CI→情景回撤 CI、年化波动 CI→夏普 CI）
- **口径修正因子** — 综合费率估算/现金剥离/TWR 计算，数据不足时回退纯说明版本
- **再平衡信号** — 单品种市值超限告警 + 权益/固收大类偏离检测，置信度分级（high/medium/low），阈值可配置（保守/稳健/进取预设）
- **流动性风险分析** — 场内品种变现天数（持仓市值/日均成交额），场外基金多日赎回天数（需配置单日上限）
- **竞争语境对比** — 组合 vs 沪深300/中证500/中证全债等多维度比较（收益、夏普、波动率、最大回撤），指数池可自定义
- **币种敞口分布** — CNY/HKD/USD 按市值加权汇总，非人民币资产>0% 时附加汇率风险提示
